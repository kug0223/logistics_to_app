...
class _WorkforceManagementScreenState extends State<WorkforceManagementScreen> {
  ...
  bool _isCalendarView = false; // false: 그룹 뷰, true: 캘린더 뷰
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_isCalendarView ? '인력 관리 - 캘린더' : '인력 관리 - 그룹'),
        backgroundColor: Colors.blue[700],
        foregroundColor: Colors.white,
        actions: [
          // 🔥 퇴사 요청 알림 아이콘
      ...
      body: _isCalendarView
          ...
          : WorkforceGroupView(businessId: _selectedBusinessId!),
    );
  }
  ...
  Future<int> _getResignRequestCount() async {
    ...
    try {
      final requests = await _firestoreService.getResignRequests(_selectedBusinessId!);  // 🔥 수정
