...
본 서비스는 만 19세 이상만 이용 가능합니다. PASS 본인인증 과정에서 만 19세 미만으로 확인될 경우 가입이 제한됩니다.
...
• 관리자의 실시간 출퇴근 현황 모니터링 (출근 중 일시적 위치 공유)
• 위치 스푸핑(Mock GPS) 감지를 통한 부정 출퇴근 방지

[수집 항목]
• GPS 위도·경도 좌표 (출퇴근 체크 시 및 출근 중 모니터링 중)
• 블루투스 기기 신호 (비콘 방식 출퇴근 사용 시에만 수집)

[이용 및 보유 기간]
• 출퇴근 시 체크한 GPS 좌표는 출퇴근 기록과 함께 5년간 보존 (근로기준법)
• 출근 중 실시간 위치 모니터링 데이터는 출근 처리 완료 즉시 수집 중지되며 별도 저장되지 않습니다.
