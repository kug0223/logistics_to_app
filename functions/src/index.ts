...
export const createNotification = onCall(
  ...
  async (request) => {
    ...
    // [FCM-02] workDate 추가 — notification_screen에서 날짜 기반 DayApplicantsDialog 분기에 필요
    const allowedDataKeys = new Set([
      ...
      "workDetailId", "workType", "expiryDate", "reason", "workDate",
    ...
    if (userId !== callerUid) {
      ...
      if (callerRole === "SUPER_ADMIN") {
        ...
        const callerIsAdmin = adminIds.includes(callerUid) ||
                              ownerId === callerUid ||
                              callerSubAdminBusinessIds.includes(targetBusinessId);

        const recipientSubAdminBusinessIds = (recipientSnap.data()?.subAdminBusinessIds ?? []) as string[];
        const recipientIsAdmin = adminIds.includes(userId) || ownerId === userId
          || recipientSubAdminBusinessIds.includes(targetBusinessId);
    ...
    // [M-7 수정 2026-08-07] 발신자별 알림 rate limit — 5분 윈도우 30건 초과 차단
    // contractRequested는 위에서 이미 24h 쿨다운으로 처리, 이중 적용 방지를 위해 제외
    if (rawType !== "contractRequested") {
      const rateLimitRef = db.collection("rate_limits").doc(`notifications_${callerUid}`);
      await db.runTransaction(async (tx) => {
        ...
        const maxCount = 30;
        if (snap.exists) {
          const windowStart = (snap.data()?.windowStart as FirebaseFirestore.Timestamp)?.toMillis() ?? 0;
          const count = (snap.data()?.count as number) ?? 0;
          if (now - windowStart < windowMs) {
            if (count >= maxCount) {
              throw new HttpsError("resource-exhausted", "알림 발송 한도를 초과했습니다. 잠시 후 다시 시도하세요.");
            }
            tx.update(rateLimitRef, {count: count + 1});
          } else {
            // 새 윈도우 시작
            tx.set(rateLimitRef, {windowStart: admin.firestore.Timestamp.fromMillis(now), count: 1});
          }
        } else {
          tx.set(rateLimitRef, {windowStart: admin.firestore.Timestamp.fromMillis(now), count: 1});
        }
    ...
    // 서브어드민 팬아웃 — 특정 관리자 타입 알림은 해당 권한을 가진 서브어드민에게도 복사
    // [팬아웃 제외] callableApplyToTO·callableRequestResignation·callableRequestInterimSettlement는
    //   각 CF 내부에서 직접 getSubAdminsWithPermission()을 호출하므로 여기서 이중 처리 불필요
    const SUB_ADMIN_FAN_OUT: Record<string, string> = {
      contractSigned:          "canManageContract",
      applicationCanceled:     "canManageTo",
      scheduleChangeRequested: "canManageWorkers",   // 클라이언트 → createNotification 경유
      contractRequested:       "canManageContract",  // 클라이언트 → createNotification 경유
      // [REG-FCM-C-01] confirmationCanceled 제거 — userId=workerUID로 발송되는 근무자용 알림이므로
      // 서브어드민 팬아웃 시 근무자용 텍스트가 관리자에게 전달되는 오발송 발생
    };
    if (rawType in SUB_ADMIN_FAN_OUT) {
      const requiredPerm = SUB_ADMIN_FAN_OUT[rawType];
      const fanoutBizId = filteredData.businessId as string | undefined;
      if (fanoutBizId) {
        try {
          const subAdminIds = await getSubAdminsWithPermission(fanoutBizId, requiredPerm);
          // [FANOUT-DUP] 이미 BUSINESS_ADMIN으로 알림을 받는 UID(businesses.adminIds) 제외 — 중복 발송 방지
          const bizSnap = await db.collection("businesses").doc(fanoutBizId).get();
          const bizAdminIds: string[] = (bizSnap.data()?.adminIds as string[] | undefined) ?? [];
          const newRecipients = subAdminIds.filter((id) => id !== userId && !bizAdminIds.includes(id));
          if (newRecipients.length > 0) {
            const batch = db.batch();
            for (const subAdminUid of newRecipients) {
              const ref = db.collection("users").doc(subAdminUid)
                .collection("notifications").doc();
              batch.set(ref, { ...payload, userId: subAdminUid });
            }
            await batch.commit();
            console.log(`🔔 [팬아웃] ${rawType}: ${newRecipients.length}명 서브어드민 알림 생성`);
          }
        } catch (e) {
          console.error(`⚠️ [팬아웃] ${rawType} 서브어드민 알림 실패:`, e);
        }
      }
    }
...
// ═══════════════════════════════════════════════════════════
// 👥 초대 수락 시 user.subAdminBusinessIds 설정 (Admin SDK, 클라이언트 규칙 우회)
// ═══════════════════════════════════════════════════════════
  ...
  async (event) => {
    ...
    try {
      await db.collection("users").doc(targetUid).update({
        ...
        subAdminOf: admin.firestore.FieldValue.delete(), // 레거시 필드 정리
      });
      console.log(`✅ [초대수락 트리거] subAdminBusinessIds 설정 완료: uid=${targetUid}, bizId=${businessId}`);
    } catch (e: any) {
...
export const onNotificationCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    // [FCM-BUG-A] 멱등성: fcmSent 여부 확인만 트랜잭션으로 처리, 플래그 저장은 발송 성공 후로 이동
    // (이전 패턴: 플래그 선저장 → FCM 실패 시 재시도 불가 → 알림 영구 누락)
    // (새 패턴: 발송 성공 후 플래그 저장 → 극히 드문 중복 발송 가능하나 누락 방지가 더 중요)
    ...
    try {
      ...
      // [F004] 중복 토큰 제거 — 동일 기기 재등록 등으로 배열에 중복 항목이 있을 경우 전송 실패 방지
      const fcmTokens: string[] = [...new Set(tokenArray && tokenArray.length > 0
        ? tokenArray
      ...
      // [FCM-BUG-A] 발송 성공 후 fcmSent 플래그 저장
      ...
      // [FCM-BUG-A] 발송 실패 시 re-throw → CF 재시도 허용 (플래그 미저장 상태이므로 안전)
      throw error;
    }
  }
);
...
export const masterScheduler = onSchedule(
  ...
  async () => {
    ...
    console.log("⏰ [시간 작업] 병렬 실행 시작 (예약공개·WorkDetail마감·슬롯마감·TO마감·초대만료)...");
    const hourlyResults = await Promise.allSettled([
      ...
      processExpiredTOInvitations(timestamp),
    ]);
    const hourlyNames = ["예약 공개", "WorkDetail 마감", "슬롯 WorkDetail 마감", "TO 마감", "초대 만료"];
...
export const onReviewCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    let blocked = false;
    // [PERF-M13] 트랜잭션 내 req 데이터 캡처 → post-tx reqRef.get() 중복 조회 제거
    let capturedReqData: admin.firestore.DocumentData | undefined;
    ...
    // 통계 업데이트 — [PERF-M13] 트랜잭션에서 캡처한 capturedReqData 재사용 (중복 reqRef.get() 제거)
    try {
      await Promise.all([
        capturedReqData?.workerId ? updateUserReviewStats(capturedReqData.workerId as string) : Promise.resolve(),
        capturedReqData?.businessId ? updateBusinessReviewStats(capturedReqData.businessId as string) : Promise.resolve(),
      ]);
    } catch (err) {
...
async function processSlotWorkDetailExpiry(now: Timestamp): Promise<void> {
  ...
  // 3단계: 결과 집계
  const totalRequiredDecrement = new Map<string, number>(); // [TO-M-06]
  for (let i = 0; i < pending.length; i++) {
    ...
    if (toId) {
      ...
      // [TO-M-06] 만료 workDetail의 requiredCount 합산 — TO.totalRequired 감소 대상
      const delta = expiredItems.reduce((acc: number, wd: any) => acc + (Number(wd.requiredCount) || 0), 0);
      if (delta > 0) totalRequiredDecrement.set(toId, (totalRequiredDecrement.get(toId) ?? 0) + delta);
    }
  }
  ...
  // [TO-M-06] TO.totalRequired 감소 — 만료된 workDetail의 requiredCount 합산
  if (totalRequiredDecrement.size > 0) {
    await Promise.allSettled(
      [...totalRequiredDecrement.entries()].map(([tid, delta]) =>
        db.collection("tos").doc(tid).update({
          totalRequired: admin.firestore.FieldValue.increment(-delta),
        }).catch((err) => console.error(`[TO-M-06] TO ${tid} totalRequired 감소 실패:`, err))
      )
    );
  }
}
...
async function syncGroupMasterStatus(
  ...
  try {
    // [PERF-M14] tos 쿼리 + groups 읽기 병렬 조회 (2 RTT → 1 RTT)
    const groupDocRef = firestore.collection("groups").doc(groupId);
    const [groupSnapshot, groupDoc] = await Promise.all([
      firestore.collection("tos").where("groupId", "==", groupId).get(),
      groupDocRef.get(),
    ]);
...
async function processContractRenewalChecks(now: Timestamp): Promise<void> {
  ...
  try {
    ...
    const d0Results = await Promise.allSettled(d0Snap.docs.map(async (doc) => {
      ...
      await renewBatch.commit(); // 새 application + 계약서 + 알림 + 기존 상태 변경 원자적 처리
      return "ok";
    }));
    ...
    // 요청일로부터 1일 경과한 신청서 대상 (내일=D+2, 모레=D+3 자동승인)
    {
      ...
      const d1Snap = await db.collection("applications")
        ...
        .limit(500).get();
      // [PERF-M12] businesses 프리패치 Map — 루프 내 개별 조회 제거
      const d1UniqueBizIds = [...new Set(d1Snap.docs.map((d) => d.data().businessId as string))];
      const d1BizSnaps = await Promise.all(d1UniqueBizIds.map((bid) => db.collection("businesses").doc(bid).get()));
      const d1BizMap = new Map(d1BizSnaps.map((snap, i) => [d1UniqueBizIds[i], snap]));
      await Promise.all(d1Snap.docs.map(async (d1Doc) => {
        try {
          const app = d1Doc.data();
          // [FCM-FIX 2026-08-10] 플래그 체크→알림 write→플래그 설정 순서로 수정
          // 이전: 트랜잭션에서 플래그 선설정 → 알림 write 실패 시 플래그 이미 커밋 → 영구 누락
          // 이후: 알림 write 성공 확인 후 플래그 설정 (매우 드문 재시도 중복 발송이 영구 누락보다 양호)
          const freshD1 = await d1Doc.ref.get();
          if (freshD1.data()?.d1NotifiedAt != null) return;

          const bizSnap = d1BizMap.get(app.businessId as string);
          ...
          // 알림 write 성공 후 플래그 설정 (실패 시 catch로 이동 → 플래그 미설정 → 다음 스케줄 재시도)
    ...
    // 요청일로부터 2일 경과한 신청서 대상 (내일=D+3 자동승인)
    {
      ...
      const d2Snap = await db.collection("applications")
        ...
        .limit(500).get();
      // [PERF-M12] businesses 프리패치 Map — 루프 내 개별 조회 제거
      const d2UniqueBizIds = [...new Set(d2Snap.docs.map((d) => d.data().businessId as string))];
      const d2BizSnaps = await Promise.all(d2UniqueBizIds.map((bid) => db.collection("businesses").doc(bid).get()));
      const d2BizMap = new Map(d2BizSnaps.map((snap, i) => [d2UniqueBizIds[i], snap]));
      await Promise.all(d2Snap.docs.map(async (d2Doc) => {
        try {
          const app = d2Doc.data();
          // [FCM-FIX 2026-08-10] D+1과 동일 순서 수정 — 알림 write 성공 후 플래그 설정
          const freshD2 = await d2Doc.ref.get();
          if (freshD2.data()?.d2NotifiedAt != null) return;

          const bizSnap = d2BizMap.get(app.businessId as string);
          const bizData = bizSnap?.exists ? bizSnap.data() : undefined;
          const adminIds: string[] = (() => {
            const ids = (bizData?.adminIds as string[] | undefined) ?? [];
            if (ids.length > 0) return ids;
    ...
    // [PERF-H1] 사업장 문서 프리패치 → 루프 내 개별 조회 제거 + Promise.allSettled 병렬화
    const resignUniqueBizIds = [...new Set(pendingResignSnap.docs.map((d) => d.data().businessId as string))];
    const resignBizSnapList = await Promise.all(resignUniqueBizIds.map((bid) => db.collection("businesses").doc(bid).get()));
    const resignBizMap = new Map(resignBizSnapList.map((snap, i) => [resignUniqueBizIds[i], snap]));

    const resignResults = await Promise.allSettled(pendingResignSnap.docs.map(async (doc) => {
      try {
        const app = doc.data();

        // actualResignDate: 근무자가 희망한 날짜 or 요청일+1일
        let actualResignDate: Date;
        // [F003] resignRequestDate가 Timestamp가 아닌 값(string 등)일 경우 TypeError 방지
        if (app.resignRequestDate instanceof Timestamp) {
          actualResignDate = app.resignRequestDate.toDate();
        } else {
        ...
        // 관리자에게도 알림 (ownerId fallback, best-effort) — 프리패치 Map에서 조회
        const bizDoc = resignBizMap.get(app.businessId as string);
    ...
    autoResignCount = resignResults.filter((r) => r.status === "fulfilled").length;
    ...
    // [PERF-H2] 사업장 문서 프리패치 → 루프 내 개별 조회 제거 + Promise.allSettled 병렬화
    const termUniqueBizIds = [...new Set(pendingTerminationSnap.docs.map((d) => d.data().businessId as string))];
    const termBizSnapList = await Promise.all(termUniqueBizIds.map((bid) => db.collection("businesses").doc(bid).get()));
    const termBizMap = new Map(termBizSnapList.map((snap, i) => [termUniqueBizIds[i], snap]));

    const termResults = await Promise.allSettled(pendingTerminationSnap.docs.map(async (doc) => {
      try {
        const app = doc.data();
        ...
        // [BUG-CF-03 수정] 관리자에게도 계약해지 자동승인 알림 — 프리패치 Map에서 조회
        const terminationBizDoc = termBizMap.get(app.businessId as string);
    ...
    autoTerminationCount = termResults.filter((r) => r.status === "fulfilled").length;
...
export const onBusinessDeleted = onDocumentDeleted(
  ...
  async (event) => {
    ...
    // [BB-001] 수락된 멤버의 subAdminBusinessIds 초기화 — [M-9 수정 2026-07-15] limit(2000) → 페이지네이션
    {
    ...
    if (purpose === "register") {
      ...
      const [dupSnap, deletedSnap] = await Promise.all([
        db.collection("users")
          ...
          .where("role", "==", role)
          .limit(1).get(),
        // [BUG-FIX] orderBy("deletedAt", "desc") — 최신 레코드를 먼저 반환
        // 무정렬 limit(1)은 구버전 isBlacklisted=false 레코드를 먼저 반환해 블랙리스트 우회 가능
      ...
      if (!deletedSnap.empty) {
        const deletedData = deletedSnap.docs[0].data();

        // [BUG-FIX] isBlacklisted → 영구 재가입 차단 (30일 제한과 별개)
        // 기존: canReregisterAt 30일만 체크 → 블랙리스트도 30일 후 재가입 가능한 버그
        if (deletedData.isBlacklisted === true) {
          throw new HttpsError(
            "permission-denied",
            "이용 정책 위반으로 재가입이 영구 제한된 계정입니다."
          );
        }

        const deletedAt = (deletedData.deletedAt as Timestamp).toDate();
...
export const callableFinalizeEmployerSignature = onCall(
  ...
  async (request) => {
    ...
    // 이후: CF 트랜잭션 성공 직후 Firestore notifications 직접 write → onNotificationCreated 트리거 → FCM
    try {
      const notifSnap = await contractRef.get();
      const nd = notifSnap.data() ?? {};
      const notifWorkerId  = nd["workerId"]     as string | undefined;
      const notifBizId     = nd["businessId"]   as string | undefined;
      const notifBizName   = nd["businessName"] as string | undefined;
      const notifAppId     = nd["applicationId"] as string | undefined;
      if (notifWorkerId) {
        await db.collection("users").doc(notifWorkerId).collection("notifications").add({
          userId:  notifWorkerId,
          type:    "contractSignRequested",
          title:   "근로계약서 서명 요청",
          body:    notifBizName
            ? `[${notifBizName}] 근로계약서를 확인하고 서명해 주세요.`
            : "근로계약서를 확인하고 서명해 주세요.",
          isRead:  false,
          data: {
            contractId,
            ...(notifBizId  ? {businessId: notifBizId}     : {}),
            ...(notifAppId  ? {applicationId: notifAppId}  : {}),
            screen: "contractSign",
          },
          createdAt: admin.firestore.FieldValue.serverTimestamp(),
          readAt:    null,
...
export const callableCreateTO = onCall(
  ...
  async (request) => {
    ...
    // [M-2-FIX] SubAdmin(role=USER, subAdminBusinessIds에 businessId 포함)도 TO 생성 허용 — callableUpdateTO/callablePublishTO와 통일
    ...
    // [TO-01] 서브어드민 canManageTo 세부 권한 CF 검증 — UI 우회 차단
    if (role !== "BUSINESS_ADMIN" && role !== "SUPER_ADMIN") {
      const memberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
      const memberPerms = (memberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!memberPerms.canManageTo) {
    ...
    if (role !== "SUPER_ADMIN") {
      ...
      if (!bizAdminIds.includes(callerUid) && bizOwnerId !== callerUid && !subAdminBusinessIds.includes(businessId)) {
...
// UTC 에뮬레이터에서 생성하면 마감시간이 +9h 오차 발생. 서버에서 KST→UTC 명시 변환으로 해결.
// Input:  { toId, businessId, dates: ["YYYY-MM-DD",...], workDetails,
//           deadlineType, hoursBeforeStart, fixedDeadlineMs?,
//           publishMode?, publishDaysBefore?, publishTime?, slotTitle? }
// Output: { slotCount: number }
export const callableCreateFlexSlots = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    ...
    // [TO-M-05] dates 입력 검증: YYYY-MM-DD 포맷 · 과거날짜 차단 · 중복 제거
    {
      ...
      const todayStr = `${todayKST.getUTCFullYear()}-` +
      ...
      for (const dateStr of dates) {
        if (typeof dateStr !== "string" || !DATE_RE.test(dateStr)) {
          throw new HttpsError("invalid-argument", `날짜 포맷 오류: ${dateStr} (YYYY-MM-DD 형식 필요)`);
        }
        if (dateStr < todayStr) {
          throw new HttpsError("invalid-argument", `과거 날짜는 등록할 수 없습니다: ${dateStr}`);
        }
      }
    }
    const uniqueDates: string[] = [...new Set(dates as string[])];
    // TO-M-01: FIXED_TIME 타입인데 fixedDeadlineMs 미전달 시 마감 없는 슬롯 생성 방지
    if (deadlineType === "FIXED_TIME" && (fixedDeadlineMs == null || fixedDeadlineMs === undefined)) {
      throw new HttpsError("invalid-argument", "deadlineType=FIXED_TIME 일 때 fixedDeadlineMs 필수입니다.");
    }
    // TO-L-01: hoursBeforeStart 범위 검증 (HOURS_BEFORE 전용)
    if (deadlineType === "HOURS_BEFORE" && (hoursBeforeStart <= 0 || hoursBeforeStart > 168)) {
      throw new HttpsError("invalid-argument", "hoursBeforeStart는 1~168 사이여야 합니다.");
    }
    // [F005] FIXED_TIME: fixedDeadline이 슬롯 시작 시간 이후면 마감이 무의미 — 차단
    if (deadlineType === "FIXED_TIME" && fixedDeadlineMs != null) {
      for (const dateStr of uniqueDates) {
        const [fy, fmo, fd] = dateStr.split("-").map(Number);
        for (const wd of workDetails) {
          if (typeof wd.startTime === "string") {
            const [fh, fm] = (wd.startTime as string).split(":").map(Number);
            const slotStartUtcMs = Date.UTC(fy, fmo - 1, fd, fh - 9, fm);
            if (fixedDeadlineMs >= slotStartUtcMs) {
              throw new HttpsError("invalid-argument",
                `fixedDeadline이 슬롯 시작 시간(${dateStr} ${wd.startTime} KST) 이후입니다.`);
            }
          }
        }
      }
    ...
    function calcDeadlineMs(dateStr: string, startTime: string, hoursBefore: number): number {
      const [y, mo, d] = dateStr.split("-").map(Number);
      const [h, m] = startTime.split(":").map(Number);
      const startUtcMs = Date.UTC(y, mo - 1, d, h - 9, m);
      return startUtcMs - hoursBefore * 60 * 60 * 1000;
    }

    // publishTime("HH:MM") KST → UTC, daysBefore일 전
    function calcVisibleFromMs(dateStr: string, daysBefore: number, pubTime: string): number {
      const [y, mo, d] = dateStr.split("-").map(Number);
      const [ph, pm] = pubTime.split(":").map(Number);
      const pubDateMs = Date.UTC(y, mo - 1, d, ph - 9, pm);
      return pubDateMs - daysBefore * 24 * 60 * 60 * 1000;
    }

    const fixedDeadline = fixedDeadlineMs != null
      ? admin.firestore.Timestamp.fromMillis(fixedDeadlineMs)
      : null;
    ...
    for (const dateStr of uniqueDates) {
      ...
      const wdsWithDeadlines = workDetails.map((wd) => {
        ...
        if (deadlineType === "HOURS_BEFORE" && typeof wd.startTime === "string") {
          newWd.applicationDeadline = admin.firestore.Timestamp.fromMillis(
            calcDeadlineMs(dateStr, wd.startTime as string, hoursBeforeStart)
          );
        } else if (deadlineType === "FIXED_TIME" && fixedDeadline) {
          newWd.applicationDeadline = fixedDeadline;
        }
        totalNewRequired += (wd.requiredCount as number ?? 0);
        return newWd;
      });

      // 슬롯 레벨 마감 = 가장 이른 업무 마감
      let slotDeadline: admin.firestore.Timestamp | null = null;
      for (const wd of wdsWithDeadlines) {
        const dl = wd.applicationDeadline as admin.firestore.Timestamp | null;
        if (dl && (!slotDeadline || dl.toMillis() < slotDeadline.toMillis())) {
          slotDeadline = dl;
        }
      }

      // visibleFrom (publishTime도 KST → UTC 서버 변환)
      let visibleFrom: admin.firestore.Timestamp | null = null;
      if (publishMode === "scheduled" && publishDaysBefore != null && publishTime) {
        visibleFrom = admin.firestore.Timestamp.fromMillis(
          calcVisibleFromMs(dateStr, publishDaysBefore, publishTime)
        );
      }
      ...
      if (slotTitle) slotData.title = slotTitle;
...
export const approveForeignWorker = onCall(
  ...
  async (request) => {
    ...
    // [PERF-M9] callerDoc + userDoc 병렬 조회 (2 RTT → 1 RTT)
    const [callerDoc, userDoc] = await Promise.all([
...
export const callableGetAdminAttendances = onCall(
  ...
  async (request) => {
    ...
    // [BUG-03] 년월 포맷 검증 강화: 구조(YYYY-MM)만 아니라 월 범위(01~12)도 검증
    const isValidYearMonth = (ym: string): boolean => {
      if (!/^\d{4}-\d{2}$/.test(ym)) return false;
      const month = parseInt(ym.slice(5, 7), 10);
      return month >= 1 && month <= 12;
    };
    const hasYearMonth = typeof filterYearMonth === "string" && isValidYearMonth(filterYearMonth);
    const hasYearMonthRange =
      typeof filterYearMonthGte === "string" && isValidYearMonth(filterYearMonthGte) &&
      typeof filterYearMonthLte === "string" && isValidYearMonth(filterYearMonthLte);
    ...
    // [BUG-02] hasPaymentDueFilter + hasDateRange 동시 사용 차단
    // workDate range + paymentDueDate range = 두 필드에 inequality → Firestore FAILED_PRECONDITION
    if (hasPaymentDueFilter && hasDateRange) {
      throw new HttpsError("invalid-argument", "paymentDueDateLteMs와 startMs/endMs(날짜 범위)를 동시에 사용할 수 없습니다.");
    }
    ...
    // [BUG-01] yearMonthRange 역전 검증
    if (hasYearMonthRange && filterYearMonthGte! > filterYearMonthLte!) {
      throw new HttpsError("invalid-argument", "yearMonthGte는 yearMonthLte 이하여야 합니다.");
    }
...
export const onBusinessDeactivated = onDocumentUpdated(
  ...
  async (event) => {
    ...
    // [PERF-H3] TO별 직렬 루프 → Promise.allSettled 병렬화 (각 TO 독립 처리)
    const BATCH_LIMIT = 400; // Firestore batch 500건 제한 안전 마진
    let totalRejected = 0;

    const toResults = await Promise.allSettled(activeTos.map(async (toDoc) => {
      const toId = toDoc.id;
...
export const callableGetUsersBatch = onCall(
  ...
  async (request) => {
    ...
    // 호출자 역할 검증
    const callerUid = request.auth.uid;
    // [PERF-UB-01] callerSnap + bizSnap 병렬 조회 — BUSINESS_ADMIN 여부와 무관하게 미리 읽어 1-RTT 절약
    const [callerSnap, bizSnap] = await Promise.all([
      db.collection("users").doc(callerUid).get(),
      db.collection("businesses").doc(businessId).get(),
    ]);
    ...
    const callerSubAdminBusinessIds: string[] = (callerData.subAdminBusinessIds ?? []) as string[];
    ...
    // SEC-26: SubAdmin은 role="USER" + subAdminBusinessIds 배열로 구별 ("SUB_ADMIN" 문자열 없음)
    const isSubAdmin = callerRole === "USER" && callerSubAdminBusinessIds.includes(businessId);
    ...
    if (noshowOn || noshowOff) {
      ...
      await db.runTransaction(async (tx) => {
        ...
        if (noshowOn) {
          ...
          // 클라이언트가 자체 취소 후 callableApplyNoShowPenalty를 이미 호출했으면 스킵
          const applicationId = after?.applicationId as string | undefined;
          if (applicationId) {
            const appSnap = await tx.get(db.collection("applications").doc(applicationId));
            if (appSnap.data()?.noShowPenaltyAppliedAt) {
              console.log(`ℹ️ [wageStatusChanged-noshow] callableApplyNoShowPenalty 이미 처리됨 — 스킵 (appId=${applicationId})`);
              tx.set(processedRef, {processedAt: admin.firestore.FieldValue.serverTimestamp()});
              return;
            }
          }
...
export const callableCancelConfirmedApplication = onCall(
  ...
  async (request) => {
    ...
    const callerSubAdminBusinessIds: string[] = (callerData?.subAdminBusinessIds ?? []) as string[];
    const isSubAdminOfBiz = callerSubAdminBusinessIds.includes(appBusinessId ?? "");
    ...
    // [SUBADMIN-PERM-02] 서브어드민 canManageTo 세부 권한 검증
    if (isSubAdminOfBiz && appBusinessId) {
      const cancelMemberSnap = await db.collection("businesses").doc(appBusinessId).collection("members").doc(callerUid).get();
      const cancelPerms = (cancelMemberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!cancelPerms.canManageTo) throw new HttpsError("permission-denied", "TO 관리 권한이 없습니다.");
    }
...
export const callableDecrementSlotConfirmed = onCall(
  ...
  async (request) => {
    ...
    const isSuperAdminCaller = callerRole === "SUPER_ADMIN";
    const appBusinessId = appData.businessId as string | undefined;
    // SubAdmin: role="USER" + subAdminBusinessIds에 businessId 포함 (firestore.rules isSubAdminOf 패턴과 동일)
    const callerSubAdminBusinessIds: string[] = (callerData?.subAdminBusinessIds ?? []) as string[];
    const isSubAdminOfBiz = callerSubAdminBusinessIds.includes(appBusinessId ?? "");
    let isAdminOfBiz = false;
    ...
    await db.runTransaction(async (tx) => {
      ...
      // [F-02] TOCTOU 방지: callableApplyToTO가 먼저 커밋해 status가 PENDING으로 전환된 경우 스킵
      const freshStatus = freshAppData.status as string | undefined;
      if (freshStatus !== "CANCELED" && freshStatus !== "AUTO_CANCELED") {
        return;
      }
...
export const callableAutoConflictCancel = onCall(
  ...
  async (request) => {
    ...
    // 1. 권한 검증 — SUPER_ADMIN / 사업장 adminIds/ownerId / SubAdmin(subAdminBusinessIds에 businessId 포함)
    const callerSnap = await db.collection("users").doc(callerUid).get();
    const callerData = callerSnap.data();
    const callerRole = callerData?.role as string | undefined;
    ...
    // SubAdmin: role="USER" + subAdminBusinessIds에 businessId 포함 (firestore.rules isSubAdminOf 패턴과 동일)
    const callerSubAdminBusinessIds: string[] = (callerData?.subAdminBusinessIds ?? []) as string[];
    const isSubAdmin = callerSubAdminBusinessIds.includes(businessId);
...
export const callableReportLate = onCall(
  ...
  async (request) => {
    ...
    } else if (!authorized && callerRole === "USER" && callerSubAdminBusinessIds.length > 0) {
      // SEC-26: SubAdmin은 role="USER" + subAdminBusinessIds 배열로 구별
      authorized = callerSubAdminBusinessIds.includes(businessId);
    }
...
function srvWageCalculate(p: {
  ...
  // [WAGE-L1] 자정 경계 보정: 야간교대(예: 예정 00:30 / 실제 23:50) 시 rawDiff가 -720분 미만이면
  // 날짜 경계를 넘어선 조기 도착으로 판단하여 +1440 보정
  const earlyRawDiff = schedStartMin - actualStartMin;
  const earlyArrivalMinutes = Math.max(0, earlyRawDiff < -720 ? earlyRawDiff + 1440 : earlyRawDiff);
...
// [PERF-H4] srvGetMonthlyWorkDaysTx + srvGetPrevGrossTotalTx 병합
// 두 함수가 동일한 3회 tx.get을 실행하여 8일차에 6회 중복 → 3회로 절감
async function srvGetMonthlyStatsTx(
  tx: FirebaseFirestore.Transaction, userId: string, biz: string, ym: string, excl: string
): Promise<{workDays: number; prevGrossTotal: number}> {
...
export const callableCalculateAndConfirmWage = onCall(
  ...
  async (request) => {
    ...
    const effectiveBaseWage = (snapshotWage != null && snapshotWage > 0) ? snapshotWage : d.baseWage;
    ...
    const callerSubAdminBusinessIds2: string[] = (callerData2.subAdminBusinessIds ?? []) as string[];
    ...
    } else if (!authorized2 && callerRole2 === "USER" && callerSubAdminBusinessIds2.length > 0) {
      // SEC-26: SubAdmin은 role="USER" + subAdminBusinessIds 배열로 구별
      authorized2 = callerSubAdminBusinessIds2.includes(businessId2);
      // [PERM-WAGE-01] 서브어드민 canManageWage 세부 권한 검증
      if (authorized2) {
        const memberSnap2 = await db.collection("businesses").doc(businessId2).collection("members").doc(callerUid).get();
        const memberPerms2 = (memberSnap2.data()?.permissions as Record<string, boolean>) ?? {};
        if (!memberPerms2.canManageWage) throw new HttpsError("permission-denied", "급여 관리 권한이 없습니다.");
      }
    }
    ...
    if (d.wageType === "daily" && minimumWage2 > 0) {
      ...
      if (schedWorkMins2 > 0) {
        ...
        if (effectiveBaseWage < minimumDailyWage) {
          throw new HttpsError("invalid-argument",
            `일급(${effectiveBaseWage}원)이 소정근로시간(${(schedWorkMins2 / 60).toFixed(1)}h) 기준 최저일급(${minimumDailyWage}원) 미만입니다.`);
        }
      }
    }
...
// ═══════════════════════════════════════════════════════════
// 🔒 공통 권한 헬퍼 — SUPER_ADMIN / adminIds / ownerId / subAdminBusinessIds
// ═══════════════════════════════════════════════════════════
...
async function getSubAdminsWithPermission(
  ...
  const usersSnap = await db
    .collection("users")
    .where("subAdminBusinessIds", "array-contains", businessId)
    .get();
  ...
  const memberSnaps = await Promise.all(
    uids.map((uid) =>
      db.collection("businesses").doc(businessId).collection("members").doc(uid).get()
    )
  );
  // 권한은 members 문서의 'permissions' 서브맵 안에 있음
  return uids.filter((_, i) => (memberSnaps[i].data()?.permissions as Record<string, unknown> | undefined)?.[permission] === true);
}
...
export const callableGetApplicationsByBiz = onCall(
  ...
  async (request) => {
    ...
    const VALID_APP_STATUSES = new Set(["PENDING", "CONTRACT_PENDING", "CONFIRMED", "REJECTED", "CANCELED", "AUTO_CANCELED", "INVITED", "EXPIRED"]); // [BUG-08 수정] 초대 상태 추가
    ...
    // [BUG-01] workDate range + workEndDate range 동시 사용 차단
    // Firestore inequality 필터는 단일 필드만 허용 — 두 range 동시 적용 시 FAILED_PRECONDITION 에러
    const hasWorkDateRange = (!workDateEqMs) && (
      (workDateGteMs && Number.isFinite(workDateGteMs)) ||
      (workDateLtMs && Number.isFinite(workDateLtMs))
    );
    const hasWorkEndDateRange =
      (workEndDateGteMs && Number.isFinite(workEndDateGteMs)) ||
      (workEndDateLtMs && Number.isFinite(workEndDateLtMs));
    if (hasWorkDateRange && hasWorkEndDateRange) {
      throw new HttpsError("invalid-argument", "workDate 범위와 workEndDate 범위를 동시에 지정할 수 없습니다.");
    }
...
export const callableGetTOsByBiz = onCall(
  ...
  async (request) => {
    ...
    // [BUG-02] status + statuses 동시 전달 차단 — else if로 statuses가 묵시적 무시되는 혼동 방지
    if (status !== undefined && statuses !== undefined) {
      throw new HttpsError("invalid-argument", "status와 statuses를 동시에 지정할 수 없습니다.");
    }
    if (status) {
    ...
    if (applicationIds && applicationIds.length > 0) {
      const snaps = await Promise.all(
        applicationIds.map((aid) =>
          // [PERF-05] applicationId:계약서 = 1:1 → limit(1)로 수정 (기존 limit(cap)은 최악 50×cap 읽기 과금)
          buildBase().where("applicationId", "==", aid).limit(1).get()
        )
      );
...
// ─── 3b. callableGetUnsentApplicationsByBiz ──────────────────────────────────
// CONTRACT_PENDING 지원서 중 활성 계약서(pending_worker/completed)가 없는 것만 반환.
// callableGetApplicationsByBiz → callableGetContractsByBiz 2-hop 직렬 호출을 1-hop으로 통합.
export const callableGetUnsentApplicationsByBiz = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    const {businessId, limit: rawLimit} = (request.data ?? {}) as {
      businessId?: string;
      limit?: number;
    };
    if (!businessId || typeof businessId !== "string" || businessId.trim().length === 0) {
    ...
    const missingNameUids = new Set<string>(
      unsentDocs
        .filter(d => !(d.data().applicantName as string | undefined) && d.data().uid)
        .map(d => d.data().uid as string),
    );
    const userNameMap = new Map<string, string>();
    if (missingNameUids.size > 0) {
      const uidList = [...missingNameUids];
      const chunks: string[][] = [];
      ...
      for (const docs of nameResults) {
        for (const d of docs) {
          if (d.exists) userNameMap.set(d.id, (d.data() as any)?.name ?? "");
        }
      }
    }

    const unsentApps = unsentDocs.map(d => {
      const data = d.data();
      const serialized = serializeFirestoreData(data);
      if (!(data.applicantName as string | undefined) && data.uid) {
        const fallback = userNameMap.get(data.uid as string);
        if (fallback) serialized.applicantName = fallback;
      }
      return {id: d.id, ...serialized};
    });
...
export const callableGetReviewRequestsByBiz = onCall(
  ...
  async (request) => {
    ...
    // [BUG-07] 잘못된 adminStatus 조용히 무시 → 전체 문서 반환 위험. throw로 변경
    const VALID_ADMIN_STATUSES = new Set(["pending", "approved", "rejected", "hidden"]);
    if (adminStatus !== undefined && !VALID_ADMIN_STATUSES.has(adminStatus as string)) {
      throw new HttpsError("invalid-argument", `허용되지 않는 adminStatus 값입니다: ${adminStatus}`);
    }
...
export const callableApproveTermination = onCall(
  ...
  async (request) => {
    ...
    if (!isWorker) {
      const {callerData: atCallerData, bizData: atBizData} = await assertBizAdmin(callerUid, businessId);
      // [SUBADMIN-PERM-06] 서브어드민 canManageWorkers 세부 권한 검증
      const atAdminIds = (atBizData?.adminIds as string[] | undefined) ?? [];
      if (
        (atCallerData?.role as string | undefined) !== "SUPER_ADMIN" &&
        !atAdminIds.includes(callerUid) &&
        (atBizData?.ownerId as string | undefined) !== callerUid
      ) {
        const atMemberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
        const atPerms = (atMemberSnap.data()?.permissions as Record<string, boolean>) ?? {};
        if (!atPerms.canManageWorkers) throw new HttpsError("permission-denied", "근로자 관리 권한이 없습니다.");
      }
    }
...
export const callableApproveResignation = onCall(
  ...
  async (request) => {
    ...
    // 권한 확인: 관리자만 가능
    const {callerData: arCallerData, bizData: arBizData} = await assertBizAdmin(callerUid, businessId);
    // [SUBADMIN-PERM-07] 서브어드민 canManageWorkers 세부 권한 검증
    const arAdminIds = (arBizData?.adminIds as string[] | undefined) ?? [];
    if (
      (arCallerData?.role as string | undefined) !== "SUPER_ADMIN" &&
      !arAdminIds.includes(callerUid) &&
      (arBizData?.ownerId as string | undefined) !== callerUid
    ) {
      const arMemberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
      const arPerms = (arMemberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!arPerms.canManageWorkers) throw new HttpsError("permission-denied", "근로자 관리 권한이 없습니다.");
    }
    // [RESIGN-HIGH-1] 자기처리 이해충돌 차단 — 서브어드민이 자신의 퇴사 신청을 직접 승인 불가
    if (callerUid === (preData.uid as string)) {
...
export const callableDeleteTO = onCall(
  ...
  async (request) => {
    ...
    const { callerData: deleteToCallerData } = await assertBizAdmin(callerUid, businessId);
    // [PERM-TO-02] 서브어드민 canManageTo 세부 권한 검증
    const deleteToCallerRole = deleteToCallerData?.role as string | undefined;
    if (deleteToCallerRole !== "BUSINESS_ADMIN" && deleteToCallerRole !== "SUPER_ADMIN") {
      const memberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
      const memberPerms = (memberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!memberPerms.canManageTo) throw new HttpsError("permission-denied", "TO 관리 권한이 없습니다.");
    }
    ...
    // [소프트 삭제 2026-08-10] slots·attendance·contracts 삭제 제거 — 모든 기록 보존
    // 1. applications 처리: 활성 → AUTO_CANCELED, 비활성은 보존 (지원 이력 유지)
    ...
    // 이유: 지원자 지원 이력, 출근 기록, 임금 정산, 계약서 등 법적 기록 보존 (근로기준법 제42조)
    // attendance·schedule_change_requests·employment_contracts 완전 보존
    ...
    for (const slotId of slotIds) {
      ...
      try {
        ...
        for (const doc of docs) {
          ...
          cancelCount++;
          if (cancelCount >= 498) {  // [SLOT-005] 1 여유 — 마지막 batch에 slot update 포함
            await cancelBatch.commit();
            cancelBatch = db.batch();
            cancelCount = 0;
          }
        }
        // [SLOT-005] slot 카운터 업데이트를 마지막 batch에 포함 — REJECTED 처리와 원자적
        cancelBatch.update(
          db.collection("tos").doc(toId).collection("slots").doc(slotId),
          slotUpdate
        );
...
export const callableDeleteSlots = onCall(
  ...
  async (request) => {
    ...
    // [SLOT-002] 실제 PENDING 지원서 수로 계산 (denormalized pendingCount는 stale 가능)
    removedPending = allActiveDocs.filter((doc) => doc.data().status === "PENDING").length;
...
export const callableRejectTermination = onCall(
  ...
  async (request) => {
    ...
    if (callerUid !== workerUid) {
      const {callerData: rtCallerData, bizData: rtBizData} = await assertBizAdmin(callerUid, businessId);
      // [SUBADMIN-PERM-08] 서브어드민 canManageWorkers 세부 권한 검증
      const rtAdminIds = (rtBizData?.adminIds as string[] | undefined) ?? [];
      if (
        (rtCallerData?.role as string | undefined) !== "SUPER_ADMIN" &&
        !rtAdminIds.includes(callerUid) &&
        (rtBizData?.ownerId as string | undefined) !== callerUid
      ) {
        const rtMemberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
        const rtPerms = (rtMemberSnap.data()?.permissions as Record<string, boolean>) ?? {};
        if (!rtPerms.canManageWorkers) throw new HttpsError("permission-denied", "근로자 관리 권한이 없습니다.");
      }
    }
...
export const callableRequestResignation = onCall(
  ...
  async (request) => {
    ...
    // 근무자 이름 조회 + 서브어드민(canManageWorkers) 병렬 조회
    const [workerSnap, subAdminIds] = await Promise.all([
      db.collection("users").doc(workerUid).get(),
      getSubAdminsWithPermission(businessId, "canManageWorkers"),
    ]);
    const workerName = (workerSnap.data()?.name as string | undefined) ?? "근무자";
    // [RESIGN-MEDIUM] workerUid 본인 제외 — 서브어드민 겸 근무자가 자기 알림 받는 것 방지
    const resignRecipients = [...new Set([...adminIds, ...subAdminIds])].filter((id) => id !== workerUid);
    ...
    // 관리자 + 서브어드민에게 알림 발송 (GCF v2 종료 전 완료 보장)
    await Promise.all(resignRecipients.map((adminUid) =>
      db.collection("users")
        .doc(adminUid)
        .collection("notifications")
        .add({
...
export const callableRejectResignation = onCall(
  ...
  async (request) => {
    ...
    // 권한: 관리자만 퇴사 거절 가능
    const {callerData: rrCallerData, bizData: rrBizData} = await assertBizAdmin(callerUid, businessId);
    // [SUBADMIN-PERM-09] 서브어드민 canManageWorkers 세부 권한 검증
    const rrAdminIds = (rrBizData?.adminIds as string[] | undefined) ?? [];
    if (
      (rrCallerData?.role as string | undefined) !== "SUPER_ADMIN" &&
      !rrAdminIds.includes(callerUid) &&
      (rrBizData?.ownerId as string | undefined) !== callerUid
    ) {
      const rrMemberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
      const rrPerms = (rrMemberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!rrPerms.canManageWorkers) throw new HttpsError("permission-denied", "근로자 관리 권한이 없습니다.");
    }
    // [RESIGN-HIGH-2] 자기처리 이해충돌 차단 — 서브어드민이 자신의 퇴사 신청을 직접 거절 불가
    if (callerUid === (preData.uid as string)) {
      throw new HttpsError("permission-denied", "자신의 퇴사 요청은 직접 처리할 수 없습니다.");
    }
...
export const callableBatchSetNoShow = onCall(
  ...
  async (request) => {
    ...
    const {callerData: nsCallerData, bizData: nsBizData} = await assertBizAdmin(callerUid, businessId);
    // [SUBADMIN-PERM-03] 서브어드민 canManageWorkers 세부 권한 검증
    const nsAdminIds = (nsBizData?.adminIds as string[] | undefined) ?? [];
    if (
      (nsCallerData?.role as string | undefined) !== "SUPER_ADMIN" &&
      !nsAdminIds.includes(callerUid) &&
      (nsBizData?.ownerId as string | undefined) !== callerUid
    ) {
      const nsMemberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
      const nsPerms = (nsMemberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!nsPerms.canManageWorkers) throw new HttpsError("permission-denied", "근로자 관리 권한이 없습니다.");
    }
...
export const callableConfirmFinalWage = onCall(
  ...
  async (request) => {
    ...
    const { callerData: confirmWageCallerData } = await assertBizAdmin(callerUid, businessId);
    // [PERM-WAGE-02] 서브어드민 canManageWage 세부 권한 검증
    const confirmWageCallerRole = confirmWageCallerData?.role as string | undefined;
    if (confirmWageCallerRole !== "BUSINESS_ADMIN" && confirmWageCallerRole !== "SUPER_ADMIN") {
      const memberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
      const memberPerms = (memberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!memberPerms.canManageWage) throw new HttpsError("permission-denied", "급여 관리 권한이 없습니다.");
    }
...
export const callableCancelFinalConfirmation = onCall(
  ...
  async (request) => {
    ...
    const callerUid = request.auth.uid;
    // [SEC-ROLE] assertBizAdmin: businesses.adminIds 기준 검증 — role="ADMIN" 오기재 수정
    const { callerData: cancelConfirmCallerData } = await assertBizAdmin(callerUid, businessId);
    // [PERM-WAGE-03] 서브어드민 canManageWage 세부 권한 검증
    const cancelConfirmCallerRole = cancelConfirmCallerData?.role as string | undefined;
    if (cancelConfirmCallerRole !== "BUSINESS_ADMIN" && cancelConfirmCallerRole !== "SUPER_ADMIN") {
      const memberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
      const memberPerms = (memberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!memberPerms.canManageWage) throw new HttpsError("permission-denied", "급여 관리 권한이 없습니다.");
    }
...
export const callableWageCancel = onCall(
  ...
  async (request) => {
    ...
    const callerUid = request.auth.uid;
    // [SEC-ROLE] assertBizAdmin: businesses.adminIds 기준 검증
    const { callerData: cancelWageCallerData } = await assertBizAdmin(callerUid, businessId);
    // [PERM-WAGE-04] 서브어드민 canManageWage 세부 권한 검증 — callableConfirmFinalWage·callableCancelFinalConfirmation과 대칭
    const cancelWageCallerRole = cancelWageCallerData?.role as string | undefined;
    if (cancelWageCallerRole !== "BUSINESS_ADMIN" && cancelWageCallerRole !== "SUPER_ADMIN") {
      const memberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
      const memberPerms = (memberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!memberPerms.canManageWage) throw new HttpsError("permission-denied", "급여 관리 권한이 없습니다.");
    }
...
export const callableBatchCheckOut = onCall(
  ...
  async (request) => {
    ...
    // Phase 2: [PERF-M3] 순차 → chunk-20 병렬 처리 (callableBatchCheckIn과 동일 패턴)
    const CHUNK_CO = 20;
...
export const callableConfirmApplication = onCall(
  ...
  async (request) => {
    ...
    if (
      (confirmCallerData?.role as string | undefined) !== "SUPER_ADMIN" &&
      !confirmAdminIds.includes(callerUid) &&
      (confirmBizData?.ownerId as string | undefined) !== callerUid
    ) {
      const confirmMemberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
      const confirmPerms = (confirmMemberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!confirmPerms.canManageTo) throw new HttpsError("permission-denied", "TO 관리 권한이 없습니다.");
    }

    // [CONFLICT-OF-INTEREST] 본인 지원서 직접 확정 차단 (서브어드민·관리자 모두 해당)
    const applicantUid = appDataPre.uid as string | undefined;
    if (applicantUid && applicantUid === callerUid) {
      throw new HttpsError(
        "permission-denied",
        "본인의 지원서는 직접 확정할 수 없습니다."
      );
    }
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (toIdFresh && selectedWorkType) {
        if (slotIdFresh) {
          ...
          if (slotFresh.exists) {
            // [SLOT-001] 마감된 슬롯에는 확정 불가
            if (slotFresh.data()?.status === "closed") {
              throw new HttpsError("failed-precondition", "마감된 슬롯에는 확정할 수 없습니다.");
            }
    ...
    const toId = appDataPre.toId as string | undefined;
    const slotId = appDataPre.slotId as string | undefined;
    const selectedWorkType = appDataPre.selectedWorkType as string | undefined;
    const uid = appDataPre.uid as string;
    const businessName = appDataPre.businessName as string;

    if (!toId) throw new HttpsError("invalid-argument", "toId가 없는 지원서입니다.");

    // ── 3. TO 로드 + 슬롯 로드 병렬 실행 + workEndDate/workDays 계산 ──
    // [PERF-02] toDoc과 slotSnap 읽기를 병렬화하여 직렬 Firestore 왕복 1회 제거
    const [toDoc, slotDocMaybe] = await Promise.all([
      db.collection("tos").doc(toId).get(),
      slotId
        ? db.collection("tos").doc(toId).collection("slots").doc(slotId).get()
        : Promise.resolve(null as admin.firestore.DocumentSnapshot | null),
    ]);
    const toData = toDoc.exists ? toDoc.data()! : undefined;

    const isLongTermApp = appDataPre.applicationType === "longTerm" ||
      (Array.isArray(appDataPre.workDays) && (appDataPre.workDays as string[]).length > 0);

    let computedWorkEndDate: admin.firestore.Timestamp | undefined;
    let computedWorkDays: string[] | undefined;

    // [M2-FIX] !appDataPre.workEndDate 조건 제거 — 클라이언트 조작값 무시, 항상 서버 TO 기준 계산
    if (isLongTermApp && toData) {
      const startTs = (appDataPre.desiredStartDate ?? appDataPre.workDate) as admin.firestore.Timestamp;
    ...
    return {
      ...
      appInfo: {
        ...
        startTime: (appDataPre.startTime as string | undefined) ?? "",
        endTime: (appDataPre.endTime as string | undefined) ?? "",
...
export const callableBatchAdjustAttendanceTime = onCall(
  ...
  async (request) => {
    ...
    const callerUid = request.auth.uid;
    // [SEC-ROLE] assertBizAdmin: businesses.adminIds 기준 검증 — role="ADMIN" 오기재 수정
    const {callerData: adjCallerData, bizData: adjBizData} = await assertBizAdmin(callerUid, businessId);
    // [SUBADMIN-PERM-04] 서브어드민 canManageWorkers 세부 권한 검증
    const adjAdminIds = (adjBizData?.adminIds as string[] | undefined) ?? [];
    if (
      (adjCallerData?.role as string | undefined) !== "SUPER_ADMIN" &&
      !adjAdminIds.includes(callerUid) &&
      (adjBizData?.ownerId as string | undefined) !== callerUid
    ) {
      const adjMemberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
      const adjPerms = (adjMemberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!adjPerms.canManageWorkers) throw new HttpsError("permission-denied", "근로자 관리 권한이 없습니다.");
    }
...
export const callableMarkTransferredBatch = onCall(
  ...
  async (request) => {
    ...
    // [SEC-ROLE] assertBizAdmin: businesses.adminIds 기준 검증 — role="ADMIN" 오기재 수정
    const {callerData: markCallerData} = await assertBizAdmin(callerUid, businessId);
    // [PAY-M1] 서브어드민 canManageWage 세부 권한 검증 — callableCancelTransfer와 대칭
    const isMarkSubAdmin = ((markCallerData?.subAdminBusinessIds ?? []) as string[]).includes(businessId);
    if (isMarkSubAdmin) {
      const memberSnap = await db.collection("businesses").doc(businessId).collection("members").doc(callerUid).get();
...
export const callableRepairPayrollSummaries = onCall(
  ...
  async (request) => {
    ...
    // [F-03] notTransferredCount 재계산 — repair 전 스테일 값이 남지 않도록
    const notTransferredCount = docs.filter(d => (d.data().wageStatus as string) === "confirmed").length;
    const summaryId = `${businessId}_${yearMonth}`;
...
// 특정 사용자에게 유효한 초대(3일 이내 PENDING)가 있는지 확인 (CF 이전 2026-07-13)
...
export const callableApproveScheduleChangeRequest = onCall(
  ...
  async (request) => {
    ...
    const scrBusinessId = scrData.businessId as string;
    const {callerData: scrCallerData, bizData: scrBizData} = await assertBizAdmin(callerUid, scrBusinessId);
    // [SUBADMIN-PERM-05] 서브어드민 canManageWorkers 세부 권한 검증
    const scrAdminIds = (scrBizData?.adminIds as string[] | undefined) ?? [];
    if (
      (scrCallerData?.role as string | undefined) !== "SUPER_ADMIN" &&
      !scrAdminIds.includes(callerUid) &&
      (scrBizData?.ownerId as string | undefined) !== callerUid
    ) {
      const scrMemberSnap = await db.collection("businesses").doc(scrBusinessId).collection("members").doc(callerUid).get();
      const scrPerms = (scrMemberSnap.data()?.permissions as Record<string, boolean>) ?? {};
      if (!scrPerms.canManageWorkers) throw new HttpsError("permission-denied", "근로자 관리 권한이 없습니다.");
    }
...
export const callableCheckIn = onCall(
  ...
  async (request) => {
    ...
    // [PERF-M2] users + applications + businesses 3개 병렬 조회 (3 RTT → 1 RTT)
    const [userSnap, appSnap, bizSnap] = await Promise.all([
      db.collection("users").doc(callerUid).get(),
    ...
    // 서버에서 사업장 attendanceRules와 지원서 startTime 직접 조회 (bizSnap은 상단 Promise.all에서 조회됨)
...
export const callableApplyToTO = onCall(
  ...
  async (request) => {
    ...
    // [CONFLICT-OF-INTEREST] 서브어드민이 자신이 관리하는 사업장 공고에 지원 차단
    // 서브어드민은 지원 확정 권한도 보유 → 자기 지원서를 직접 확정하는 이해충돌 방지
    const callerSubAdminBizIds =
      (userData["subAdminBusinessIds"] as string[] | undefined) ?? [];
    if (callerSubAdminBizIds.includes(businessId)) {
      throw new HttpsError(
        "permission-denied",
        "관리자로 등록된 사업장의 공고에는 지원할 수 없습니다."
      );
    }
    // [D8-06] BUSINESS_ADMIN이 자신이 adminIds/ownerId로 등록된 사업장 공고에 지원 차단
    if (userData["role"] === "BUSINESS_ADMIN") {
      const bizForConflictSnap = await db.collection("businesses").doc(businessId).get();
      // [CF-SEC-01] 삭제된 사업장 TO 잔존 시 이해충돌 차단 우회 방지 — 명시적 not-found 반환
      if (!bizForConflictSnap.exists) {
        throw new HttpsError("not-found", "사업장 정보를 찾을 수 없습니다.");
      }
      const bizAdminIds = (bizForConflictSnap.data()?.adminIds as string[] | undefined) ?? [];
      const bizOwnerId = bizForConflictSnap.data()?.ownerId as string | undefined;
      if (bizAdminIds.includes(uid) || bizOwnerId === uid) {
        throw new HttpsError(
          "permission-denied",
          "관리자로 등록된 사업장의 공고에는 지원할 수 없습니다."
        );
    ...
    if (slotId) {
      ...
      const slotWorkDateTs = sd["workDate"] as admin.firestore.Timestamp | undefined;
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (txIsReactivation) {
        const reactivateData: Record<string, unknown> = {
          ...
          // [APP-01] 재지원 시 멱등성 마커 초기화 — 2회차 취소 시 슬롯 카운터 감소 정상 동작 보장
          confirmedDecrementedAt: admin.firestore.FieldValue.delete(),
        };
    ...
    try {
      ...
      // 서브어드민(canManageTo 권한)도 수신자에 포함
      const subAdminIds = await getSubAdminsWithPermission(businessId, "canManageTo");
      const allRecipients = [...new Set([...adminIds, ...subAdminIds])];
      console.log(`🔔 [applyToTO] bizExists=${bizSnap.exists}, adminIds=${JSON.stringify(adminIds)}, subAdminIds=${JSON.stringify(subAdminIds)}`);
      if (allRecipients.length > 0) {
        ...
        await Promise.all(allRecipients.map((adminId) =>
          db.collection("users").doc(adminId).collection("notifications").add({
            userId: adminId,
            type: "newApplication",
            title: "새 지원서 접수",
        ...
        console.log(`🔔 [applyToTO] newApplication 알림 → ${allRecipients.length}명`);
      }
    } catch (notifErr) {
...
// ─── callableGetAllBusinesses ────────────────────────────────────────────────
// [SA-01-CF] 슈퍼어드민 전용 사업장 목록 조회 — Admin SDK로 rate limiting 강제
// Trust Boundary Charter: businesses 컬렉션에 adminIds(UID 목록) 포함 → CF 전용
// ─────────────────────────────────────────────────────────────────────────────
export const callableGetAllBusinesses = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    ...
    const businesses = snap.docs.map((doc) => ({id: doc.id, ...serializeFirestoreData(doc.data())}));
    return {businesses};
  }
);
...
// ─── callableGetSentPendingInvitations ───────────────────────────────────────
// BUSINESS_ADMIN이 특정 사업장에 발송한 pending 초대 목록 (3일 이내만)
// [CF 이전 2026-07-27] member_invitations list 쿼리 rules PERMISSION_DENIED → Admin SDK 이전
export const callableGetSentPendingInvitations = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const uid = request.auth.uid;
...
// ─── callableAcceptInvitation ─────────────────────────────────────────────────
// [CF-MIGRATED D4-CP1] acceptInvitation CF 이전 — 3일 만료 서버 강제
// 클라이언트 직접 Firestore 배치에서 Admin SDK로 이전 → rules 30일 루프홀 차단
export const callableAcceptInvitation = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    ...
    if (!invitationId || typeof invitationId !== "string") {
      throw new HttpsError("invalid-argument", "invitationId가 필요합니다.");
    }
    ...
    try {
      ...
      if (adminUid) {
        await db.collection("users").doc(adminUid).collection("notifications").add({
          userId: adminUid,
          type: "memberInvitationAccepted",
          title: "초대 수락됨",
          body: `${inv.targetName ?? "근무자"}님이 ${inv.businessName ?? ""}${inv.businessName ? " " : ""}관리자 초대를 수락했습니다.`,
          data: {action: "memberManagement", businessId},
          isRead: false,
          createdAt: admin.firestore.Timestamp.now(),
        });
      }
...
// ════════════════════════════════════════════════════════════════════════════
// 🎫 근로자 TO 초대 시스템
//    processExpiredTOInvitations  — 시간별 스케줄러: INVITED → EXPIRED 자동 만료
//    callableInviteWorker         — 관리자가 근로자에게 특정 TO 초대 발송
//    callableAcceptTOInvitation   — 근로자가 초대 수락 (INVITED → CONFIRMED)
//    callableDeclineTOInvitation  — 근로자가 초대 거절 (INVITED → REJECTED)
// ════════════════════════════════════════════════════════════════════════════

// ── processExpiredTOInvitations ───────────────────────────────────────────────
// masterScheduler 시간별 작업: inviteExpiresAt <= now 인 INVITED 지원서 → EXPIRED
async function processExpiredTOInvitations(now: admin.firestore.Timestamp): Promise<void> {
...
// 관리자가 특정 근로자에게 TO 근무 초대를 발송한다.
// Input:
//   toId         string   — 공고 ID
//   businessId   string   — 사업장 ID
//   targetUid    string   — 초대 대상 근로자 UID
//   slotId?      string   — 단기 TO 슬롯 ID
//   workDate     string   — 단기: 근무일 ISO / 장기: 첫 출근일 ISO
//   workEndDate? string   — 장기 TO 종료일 ISO
//   workDays?    string[] — 장기 TO 근무 요일 (TO.workDays 그대로)
// Output: { success, applicationId }
export const callableInviteWorker = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    ...
    if (!businessId || typeof businessId !== "string") throw new HttpsError("invalid-argument", "businessId가 필요합니다.");
    ...
    if (targetUid === callerUid) {
      throw new HttpsError("invalid-argument", "자기 자신에게는 초대를 보낼 수 없습니다.");
    }

    // ── 3. 대상 근로자 존재 및 역할 확인 ─────────────────────────────────────
    const targetSnap = await db.collection("users").doc(targetUid).get();
    if (!targetSnap.exists) throw new HttpsError("not-found", "대상 사용자를 찾을 수 없습니다.");
    const targetData = targetSnap.data()!;
    const targetName = (targetData.name as string | undefined) ?? "근로자";

    // [M-3 수정] 관리자·슈퍼어드민에게 초대 발송 차단 — USER 역할만 허용
    ...
    const toStatus = (toData.status as string | undefined) ?? "";
    const closedStatuses = ["CLOSED", "POSTING_EXPIRED", "DELETED"];
    if (closedStatuses.includes(toStatus)) {
      throw new HttpsError("failed-precondition", "마감된 공고에는 초대를 보낼 수 없습니다.");
    }
    // [BUG-09 수정] FULL 상태에서도 초대 차단 — 수락 시 totalConfirmed 오버부킹 방지
    // callableApplyToTO는 FULL을 차단하지만 callableInviteWorker에는 없었음
    if (toStatus === "FULL") {
      throw new HttpsError("failed-precondition", "정원이 마감된 공고에는 초대를 보낼 수 없습니다.");
    }
    ...
    if (slotId) {
      ...
      if (slotSnap.exists) {
        startTime = (slotSnap.data()!.startTime as string | undefined) ?? startTime;
        endTime   = (slotSnap.data()!.endTime   as string | undefined) ?? endTime;
    ...
    const expiresAt  = admin.firestore.Timestamp.fromMillis(
      inviteTime.toMillis() + 24 * 60 * 60 * 1000  // +24시간
    );

    const workDateTs    = admin.firestore.Timestamp.fromDate(new Date(workDate));
    const workEndDateTs = workEndDate
      ? admin.firestore.Timestamp.fromDate(new Date(workEndDate))
      : null;
    // [BUG-12 수정] workDays 빈 배열 장기 TO 오분류 방지
    // workEndDate가 있거나 workDays 요일이 있으면 장기 TO로 판별
    const isLong = !slotId && (
      (typeof workEndDate === "string" && workEndDate.length > 0) ||
    ...
    const appData: Record<string, unknown> = {
      toId,
      businessId,
      businessName: (invBizData?.name as string | undefined) ?? "",
      toTitle:      (toData.title     as string | undefined) ?? "",
      uid:          targetUid,
      applicantName: targetName,
      status:        "INVITED",
      type: isLong ? "long_term" : "short",  // [BUG-01 수정] applicationType(X)→type(O), "longTerm"→"long_term", "shortTerm"→"short"
      startTime,
      endTime,
      workDate: workDateTs,
...
// Input : { applicationId }
// Output: { success, alreadyConfirmed? }
export const callableAcceptTOInvitation = onCall(
  ...
  async (request) => {
    ...
    // ── 1. 초대 대상 본인 검증 ───────────────────────────────────────────────
    if ((appData.uid as string | undefined) !== callerUid) {
      throw new HttpsError("permission-denied", "본인의 초대만 수락할 수 있습니다.");
    }
    ...
    if (currentStatus === "REJECTED") {
      throw new HttpsError("failed-precondition", "이미 거절한 초대입니다.");
    }
    if (currentStatus !== "INVITED") {
      throw new HttpsError("failed-precondition", `수락할 수 없는 상태입니다: ${currentStatus}`);
    }
    ...
    const workDateTs    = appData.workDate    as admin.firestore.Timestamp | undefined;
    const workEndDateTs = appData.workEndDate as admin.firestore.Timestamp | undefined;
    const workDays      = (appData.workDays   as string[] | undefined) ?? [];
    // [BUG-12 수정] type 필드 우선 — workDays 빈 배열 장기 TO도 정확하게 판별
    const appType       = (appData.type as string | undefined) ?? "";
    const isLong        = appType === "long_term" || workDays.length > 0;
    const KST_OFFSET_MS = 9 * 60 * 60 * 1000;
    const weekdayNames  = ["일", "월", "화", "수", "목", "금", "토"];

    if (startTime && endTime && workDateTs) {
      const [confirmedSnap, contractSnap] = await Promise.all([
        db.collection("applications").where("uid", "==", callerUid).where("status", "==", "CONFIRMED").limit(200).get(),
        db.collection("applications").where("uid", "==", callerUid).where("status", "==", "CONTRACT_PENDING").limit(200).get(),
      ]);
      const confirmedApps: Array<Record<string, unknown>> = [...confirmedSnap.docs, ...contractSnap.docs]
        .map((d) => ({id: d.id, ...(d.data() as Record<string, unknown>)}));

      const workDateDate = workDateTs.toDate();
      const toDateOnly   = (d: Date) => new Date(d.getFullYear(), d.getMonth(), d.getDate());

      for (const app of confirmedApps) {
        const appStart = (app["startTime"] as string | undefined) ?? "";
        const appEnd   = (app["endTime"]   as string | undefined) ?? "";
        if (!_hasTimeOverlap(startTime, endTime, appStart, appEnd)) continue;

        const appWorkDateTs = app["workDate"] as admin.firestore.Timestamp | undefined;
        if (!appWorkDateTs) continue;
        const appWorkDate = appWorkDateTs.toDate();
        const appIsLong   = Array.isArray(app["workDays"]) && (app["workDays"] as string[]).length > 0;
        const appWorkDays = (app["workDays"] as string[] | undefined) ?? [];

        if (isLong) {
          const newStart = toDateOnly(workDateDate);
          const newEnd   = workEndDateTs ? toDateOnly(workEndDateTs.toDate()) : new Date(9999, 11, 31);
          if (appIsLong) {
            const appEndTs = app["workEndDate"] as admin.firestore.Timestamp | undefined;
            const sStart   = toDateOnly(appWorkDate);
            const sEnd     = toDateOnly(appEndTs?.toDate() ?? new Date(9999, 11, 31));
            if (newEnd < sStart || newStart > sEnd) continue;
            const conflictDay = workDays.find((d) => appWorkDays.includes(d));
            if (!conflictDay) continue;
            throw new HttpsError("permission-denied", `매주 ${conflictDay}에 ${appStart}~${appEnd} 확정된 근무가 있어 수락할 수 없습니다.`);
          } else {
            const sDate = toDateOnly(appWorkDate);
            if (sDate < newStart || sDate > newEnd) continue;
            const kst     = new Date(appWorkDate.getTime() + KST_OFFSET_MS);
            const weekday = weekdayNames[kst.getUTCDay()];
            if (!workDays.includes(weekday)) continue;
            throw new HttpsError("permission-denied", `${kst.getUTCMonth() + 1}/${kst.getUTCDate()}에 ${appStart}~${appEnd} 확정된 근무가 있어 수락할 수 없습니다.`);
          }
        } else {
          const workDateOnly = toDateOnly(workDateDate);
          if (appIsLong) {
            ...
            const sStart   = toDateOnly(appWorkDate);
            const sEnd     = toDateOnly(appEndTs?.toDate() ?? new Date(9999, 11, 31));
            if (workDateOnly < sStart || workDateOnly > sEnd) continue;
            const kst     = new Date(workDateDate.getTime() + KST_OFFSET_MS);
            const weekday = weekdayNames[kst.getUTCDay()];
            if (!appWorkDays.includes(weekday)) continue;
            throw new HttpsError("permission-denied", `이미 ${appStart}~${appEnd}에 확정된 근무가 있습니다.`);
          } else {
            if (toDateOnly(appWorkDate).getTime() !== workDateOnly.getTime()) continue;
            throw new HttpsError("permission-denied", `해당 날짜 ${appStart}~${appEnd}에 이미 확정된 근무가 있습니다.`);
          }
        }
      }
    }

    // ── 5. INVITED → CONFIRMED 트랜잭션 ─────────────────────────────────────
    const toId        = appData.toId   as string | undefined;
    const slotId      = appData.slotId as string | undefined;
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (toId) {
        const freshTo = await tx.get(db.collection("tos").doc(toId));
        if (!freshTo.exists) {
          throw new HttpsError("failed-precondition", "공고가 삭제되어 초대를 수락할 수 없습니다.");
        }
        const freshToStatus = (freshTo.data()!.status as string | undefined) ?? "";
        const invalidStatuses = ["CLOSED", "POSTING_EXPIRED", "DELETED", "FULL"];
...
// ── callableDeclineTOInvitation ───────────────────────────────────────────────
// 근로자가 TO 초대를 거절한다. INVITED → REJECTED
// Input : { applicationId }
// Output: { success }
export const callableDeclineTOInvitation = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    ...
    // ── 1. 초대 대상 본인 검증 ───────────────────────────────────────────────
    if ((appData.uid as string | undefined) !== callerUid) {
      throw new HttpsError("permission-denied", "본인의 초대만 거절할 수 있습니다.");
    }

    // ── 2. 상태 검증 ─────────────────────────────────────────────────────────
    const currentStatus = appData.status as string | undefined;
    if (currentStatus === "REJECTED") return {success: true};
    if (currentStatus === "CONFIRMED") {
      throw new HttpsError("failed-precondition", "이미 수락된 초대는 거절할 수 없습니다.");
    }
    if (currentStatus !== "INVITED" && currentStatus !== "EXPIRED") {
      throw new HttpsError("failed-precondition", `처리할 수 없는 상태입니다: ${currentStatus}`);
    }
    ...
    try {
      ...
      if (invitedBy) {
        db.collection("users").doc(invitedBy).collection("notifications").add({
          ...
          createdAt: admin.firestore.FieldValue.serverTimestamp(),
          readAt:    null,
        }).catch((err) => console.error("[callableDeclineTOInvitation] 관리자 알림 실패:", err));
      }
...
export const callableDeleteAccountApplications = onCall(
  ...
  async (request) => {
    ...
    // [BUG-C027 FIX] Batch 500건 한도 초과 방지 — 499건씩 청크 분할
    //   지원서(최대 ~300) + 출근부(최대 ~수백) 합계 501건 이상 시 단일 batch.commit() 크래시
    if (allUpdates.length > 0) {
      const CHUNK = 499;
      for (let i = 0; i < allUpdates.length; i += CHUNK) {
        const chunkBatch = db.batch();
        for (const {ref, data} of allUpdates.slice(i, i + CHUNK)) {
    ...
    // ── Counter 감소 내장 ────────────────────────────────────────────────────
    // [CF-MERGED] 기존 클라이언트 처리 2종 → CF 내부로 통합 (CF 호출 N+1건 절약)
    ...
    if (pendingToIds.length > 0) {
      try {
        ...
        console.error(`callableDeleteAccountApplications: totalPending decrement failed:`, e);
      }
    }

    // (B) CONFIRMED/CONTRACT_PENDING totalConfirmed: 병렬 GET + 단일 batch (음수방지 + 멱등)
    //     [PERF] 기존 직렬 트랜잭션 K건 → 병렬 GET + batch 1회로 전환 (K=50: 7.5s → ~0.1s)
    //     Admin SDK 환경: rules ±1 제한 없음 → 트랜잭션 불필요, batch로 충분
    //     멱등: 재시도 시 applications가 이미 CANCELED → 초기 쿼리에서 제외 → 이 블록 자체 미실행
    if (confirmedApps.length > 0) {
      try {
        ...
        for (const app of confirmedApps) {
          if (!toGroups.has(app.toId!)) {
            toGroups.set(app.toId!, {totalDecrement: 0, slots: new Map(), flexWorkTypes: new Map()});
          }
          const group = toGroups.get(app.toId!)!;
          group.totalDecrement += 1;
          ...
          if (app.slotId) {
            ...
            group.flexWorkTypes.set(
              app.workType,
              (group.flexWorkTypes.get(app.workType) ?? 0) + 1
            );
          }
        }

        // 2. 모든 TO 병렬 GET (음수방지용 현재값 확인)
        const toEntries = [...toGroups.entries()];
        const toSnaps = await Promise.all(
          toEntries.map(([toId]) => db.collection("tos").doc(toId).get())
        );

        // 3. 슬롯이 있는 경우 슬롯도 병렬 GET
        type SlotFetch = {
          toId: string; slotId: string;
          ref: FirebaseFirestore.DocumentReference;
        };
        const slotFetches: SlotFetch[] = [];
        for (const [toId, group] of toGroups) {
          for (const slotId of group.slots.keys()) {
            slotFetches.push({
              toId, slotId,
              ref: db.collection("tos").doc(toId).collection("slots").doc(slotId),
            });
          }
        }
        const slotSnaps = slotFetches.length > 0
          ? await Promise.all(slotFetches.map((sf) => sf.ref.get()))
          : [];
        ...
        for (let i = 0; i < toEntries.length; i++) {
          ...
          const toSnap = toSnaps[i];
          const toRef = db.collection("tos").doc(toId);
          const currentConfirmed = (toSnap.data()?.totalConfirmed as number) ?? 0;
          const actualDecrement = Math.min(group.totalDecrement, currentConfirmed);

          if (actualDecrement > 0) {
            const toUpdate: Record<string, unknown> = {
              totalConfirmed: admin.firestore.FieldValue.increment(-actualDecrement),
            };
            // flex TO workType 서브카운터 (슬롯 없는 TO)
            for (const [wt, cnt] of group.flexWorkTypes.entries()) {
              const wtCounts =
                (toSnap.data()?.workTypeConfirmedCounts as Record<string, number> | undefined) ?? {};
              const actualWtDecrement = Math.min(cnt, wtCounts[wt] ?? 0);
              if (actualWtDecrement > 0) {
                toUpdate[`workTypeConfirmedCounts.${wt}`] =
          ...
          for (const [slotId, slotGroup] of group.slots.entries()) {
            ...
            const slotRef = toRef.collection("slots").doc(slotId);
            const slotConfirmed = (slotSnap?.data()?.confirmedCount as number) ?? 0;
    ...
    // ── PENDING 변경 요청 취소 ─────────────────────────────────────────────────
    // [BUG-05 FIX] schedule_change_requests / payment_change_requests PENDING 고아
    //   기존: 미처리 → 관리자 화면에 탈퇴 사용자 요청 잔류, 처리 시 users 조회 실패 가능
    //   변경: 탈퇴 시 PENDING 상태 요청을 canceled(USER_DELETED)로 처리
    try {
      const [schedSnap, payChgSnap] = await Promise.all([
        db.collection("schedule_change_requests")
          .where("applicantUid", "==", callerUid)
          .where("status", "==", "pending")
          .get(),
        db.collection("payment_change_requests")
          .where("workerId", "==", callerUid)
          .where("status", "==", "pending")
          .get(),
      ]);
      const changeReqDocs = [...schedSnap.docs, ...payChgSnap.docs];
      if (changeReqDocs.length > 0) {
        const CHUNK = 499;
        for (let i = 0; i < changeReqDocs.length; i += CHUNK) {
          const reqBatch = db.batch();
          for (const doc of changeReqDocs.slice(i, i + CHUNK)) {
            reqBatch.update(doc.ref, {
              status: "canceled",
      ...
      console.error(`callableDeleteAccountApplications: change request cancellation failed (${callerUid}):`, e);
    }
...
// 탈퇴 처리: 탈퇴기록 저장 + 개인정보 익명화 & 연관 문서 삭제 (병렬)
// [CF-MERGED] recordDeletedAccount 통합 — CF 호출 1건 절약 (클라이언트 4→3 CF 호출)
...
export const callableDeleteAccountPreData = onCall(
  ...
  async (request) => {
    ...
    // [CF-MERGED] recordDeletedAccount 로직 — users 문서가 아직 존재할 때 선행 실행
    // users 문서는 callableDeleteAccountFinal에서 삭제 → 이 단계에서 ciHash/phone 읽기 가능
    // [AUTH-H2] Admin SDK → deleted_accounts if false 규칙 우회 (클라이언트 직접 쓰기 차단 유지)
    // [BUG-H023 FIX] 블랙리스트 사용자 기록 실패 시 throw (영구 차단 보장)
    //   일반 사용자: best-effort(30일 제한 실패 허용)
      ...
      try {
        ...
        if (userSnapForRecord.exists) {
          ...
          const phoneHash = phone
            ? crypto.createHash("sha256").update(phone).digest("hex")
            : undefined;
          isBlacklisted = userData["isBlacklisted"] === true;
          if (ciHash || phoneHash) {
            // [E-015/H-019 FIX] 멱등 체크 — 재시도 시 deleted_accounts 중복 생성 방지
            //   PreData CF 재시도 시 동일 uid로 add()가 두 번 실행되어 레코드가 복수 생성됨
            //   → uid 기준 기존 레코드 확인 후 없을 때만 생성
            const existingSnap = await db.collection("deleted_accounts")
              .where("uid", "==", uid)
              .limit(1)
              .get();
            if (!existingSnap.empty) {
              console.info(`callableDeleteAccountPreData: deleted_accounts already exists (${uid}), skip`);
            } else {
              const recordDocData: Record<string, unknown> = {
                uid,
                deletedAt: admin.firestore.FieldValue.serverTimestamp(),
                isBlacklisted,
              ...
              if (ciHash) recordDocData["ciHash"] = ciHash;
              if (phoneHash) recordDocData["phoneHash"] = phoneHash;
              if (!isBlacklisted) {
                recordDocData["canReregisterAt"] = admin.firestore.Timestamp.fromDate(
                  new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
                );
              }
              await db.collection("deleted_accounts").add(recordDocData);
            }
          }
        }
      } catch (e) {
        ...
        // 일반 사용자: best-effort — 30일 재가입 제한 실패는 허용
      }
    }
    ...
    const results = await Promise.allSettled([
      ...
      // [BUG-E007 FIX] workerId: fvDel() 추가 — 누락 시 100건 초과 페이지네이션 무한루프
      //   workerId가 남아 있으면 다음 while 반복에서 동일 문서가 계속 조회됨
      paginateUpdate("review_requests", "workerId", {workerName: "탈퇴한 회원", workerId: fvDel()}),
...
// ─── callableDeleteAccountFinal ──────────────────────────────────────────────
// 탈퇴 최종 단계: users 문서 삭제 → Storage 정리 → notifications 정리 → businesses 정리 → Auth 삭제
// 반드시 callableDeleteAccountPreData / callableDeleteAccountApplications /
// callableDeleteWorkerLocations 완료 후 마지막에 호출
//
// [CF-FINAL] 핵심 안전 보장:
//   · Admin SDK auth.deleteUser() 실행 전 모든 cleanup 완료 → 불완전 탈퇴 방지
//   · adminIds array-contains 쿼리로 복수 사업장 전체 처리 (기존 client단 단수 처리 한계 해소)
//   · auth_time 체크로 재인증 최근성 보장 (10분)
//   · 멱등 처리: 재시도 시 이미 삭제된 상태도 안전하게 진행
export const callableDeleteAccountFinal = onCall(
  {region: "asia-northeast3", enforceAppCheck: true, timeoutSeconds: 300},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const uid = request.auth.uid;
    ...
    //      [PERF] worker_locations 삭제를 users GET과 병렬화 — ~0.3s 단축
    const userRef = db.collection("users").doc(uid);
    const [, userSnap] = await Promise.all([
      // 0. worker_locations 전체 삭제 (SEC-94 위치정보 즉시 파기)
      //    [CF-MERGED] callableDeleteWorkerLocations 통합 — CF 호출 1건 절약
      //    위치정보: 개인정보보호법 제21조 (법령 보존 근거 없음 — 즉시 파기 대상)
      (async () => {
        try {
          let wlHasMore = true;
          while (wlHasMore) {
            const wlSnap = await db.collection("worker_locations")
              .where("userId", "==", uid)
              .limit(100)
              .get();
            if (wlSnap.docs.length === 0) break;
            wlHasMore = wlSnap.docs.length === 100;
            const wlBatch = db.batch();
            for (const doc of wlSnap.docs) wlBatch.delete(doc.ref);
            await wlBatch.commit();
          }
        } catch (e) {
          console.error(`callableDeleteAccountFinal: worker_locations cleanup failed (${uid}):`, e);
          // best-effort — 위치정보 정리 실패도 Auth 삭제는 진행
        }
      })(),
      // 1. users 문서 조회 (role 파악 — BUSINESS_ADMIN 분기용)
      //    멱등: 이미 삭제된 경우 role = "" 로 진행
      userRef.get(),
    ]);
    ...
    // [BUG-01 FIX] 재시도 시 users 문서가 이미 삭제된 경우 → deleted_accounts에서 role 복구
    // 재시도 케이스: users 삭제 성공 → businesses 정리 실패 → Auth 삭제 실패 → 재시도
    //   이 시점에서 userSnap.exists = false → role = "" → BUSINESS_ADMIN 분기 미실행
    // 대응: PreData CF가 deleted_accounts.role을 저장하므로 여기서 참조 (uid 인덱스 활용)
    if (!userSnap.exists) {
      try {
        const deletedAccSnap = await db.collection("deleted_accounts")
          .where("uid", "==", uid)
          .limit(1)
          .get();
        if (!deletedAccSnap.empty) {
          role = (deletedAccSnap.docs[0].data()["role"] as string | undefined) ?? "";
          console.info(`callableDeleteAccountFinal: role recovered from deleted_accounts (${uid}): ${role}`);
        }
      } catch (e) {
        console.warn(`callableDeleteAccountFinal: role recovery from deleted_accounts failed (${uid}):`, e);
      }
    }

    // 2-PRE. members 서브컬렉션 orphan 전수 정리 (best-effort)
    //   Auth 삭제 후 businesses/members 도큐먼트가 잔류해도 보안 위험 없음(인증 불가)
    //   그러나 관리자 멤버 목록에 유령 항목이 남으므로 데이터 위생 차원에서 정리.
    // [BUG-03 FIX] 기존: subAdminBusinessIds 배열 기준(서브관리자 only) → 일반 멤버 고아 미정리
    //   변경: collectionGroup("members").where("uid", "==", uid) 전수 정리
    //   callableAcceptInvitation이 members 문서에 uid 필드를 저장하므로 쿼리 가능
    //   userSnap 존재 여부와 무관하게 실행 → 재시도 시에도 안전 (BUG-01 연계)
    try {
      const memberGroupSnap = await db.collectionGroup("members")
        .where("uid", "==", uid)
        .get();
      if (memberGroupSnap.docs.length > 0) {
        const MEMBER_CHUNK = 499;
        for (let i = 0; i < memberGroupSnap.docs.length; i += MEMBER_CHUNK) {
          const memberBatch = db.batch();
          for (const doc of memberGroupSnap.docs.slice(i, i + MEMBER_CHUNK)) {
      ...
      console.error(`[callableDeleteAccountFinal] members collectionGroup cleanup failed (${uid}):`, e);
    }

    // 2-PRE2. trust_settings/{uid} 삭제 (개인정보보호법 §21, best-effort)
    // [BUG-04 FIX] trust_settings 문서는 uid가 문서 ID — 기존 탈퇴 CF에서 누락
    await db.collection("trust_settings").doc(uid).delete().catch((e) =>
      console.error(`callableDeleteAccountFinal: trust_settings cleanup failed (${uid}):`, e)
    );

    // 2. users Firestore 문서 삭제 (개인정보보호법 §21)
    if (userSnap.exists) {
      await userRef.delete();
    }

    // 3+4. Storage 정리 + notifications 삭제 병렬 실행 (상호 독립, best-effort)
    //      [PERF] 두 paginated 작업을 병렬화 — ~0.5s 단축
    await Promise.all([
      // 3. Storage users/ 정리 (신분증·서명 이미지)
      (async () => {
        try {
          const bucket = admin.storage().bucket();
          ...
          if (files.length > 0) {
            await Promise.all(
              files.map((f) =>
                f.delete().catch((e) => {
                  console.warn(`callableDeleteAccountFinal: Storage file skip (${f.name}):`, e);
                })
              )
            );
          }
        } catch (e) {
          console.error(`callableDeleteAccountFinal: Storage cleanup failed (${uid}):`, e);
          // continue — 파일 고아 잔류는 허용, Auth 삭제 우선
        }
      })(),
      // 4. notifications 서브컬렉션 삭제 (paginated 200건씩)
      //    Firestore: 상위 document 삭제 후에도 서브컬렉션 독립 접근 가능 → 순서 무관
      (async () => {
        try {
          const notifRef = db.collection("users").doc(uid).collection("notifications");
          let hasMore = true;
          while (hasMore) {
            const snap = await notifRef.limit(200).get();
            if (snap.empty) break;
          ...
          console.error(`callableDeleteAccountFinal: Notifications cleanup failed (${uid}):`, e);
        }
      })(),
    ]);

    // 5. BUSINESS_ADMIN 사업장 정리
    //    [M-01 수정] adminIds array-contains 쿼리 → 복수 사업장 전체 처리
    //    (기존 클라이언트: businessId 단수 필드만 → 공동 관리 사업장 누락 버그)
    if (role === "BUSINESS_ADMIN" || role === "SUPER_ADMIN") {
      try {
        ...
        for (const bizDoc of bizSnap.docs) {
          if (bizDoc.data()["deactivatedAt"]) continue; // 이미 비활성화 — 멱등
          const adminIds =
            (bizDoc.data()["adminIds"] as string[] | undefined) ?? [];
          const isSoleAdmin =
            adminIds.length === 1 && adminIds.includes(uid);

          if (isSoleAdmin) {
            // 단독 관리자 → 사업장 비활성화
            // onBusinessDeactivated CF가 활성 TO CLOSED + PENDING 지원서 처리를 자동으로 수행
            await bizDoc.ref.update({
              deactivatedAt: admin.firestore.FieldValue.serverTimestamp(),
            });
          } else {
            // 공동 관리자 → adminIds에서 본인 uid만 제거
            await bizDoc.ref.update({
        ...
        console.error(`callableDeleteAccountFinal: Businesses cleanup failed (${uid}):`, e);
        // continue — 사업장 정리 부분 실패도 Auth 삭제는 진행
      }
    }

    // 6. Firebase Auth 계정 삭제 (Admin SDK) — 마지막, 멱등
    //    auth.deleteUser()가 마지막이므로: 앞 단계 실패 시 사용자가 재시도 가능
    try {
      ...
      if (errCode === "auth/user-not-found") {
        // 이미 삭제됨 — 멱등 OK
        console.info(`callableDeleteAccountFinal: Auth already deleted (${uid})`);
      } else {
        throw authErr; // 예상치 못한 오류 → 클라이언트 전달
      }
    }
...
// ─── callableGetWithdrawBlockers ──────────────────────────────────────────────
// 탈퇴 전 차단/경고 조건 서버 체크 — 관리자·근로자 공통
// BUSINESS_ADMIN: 미확정/미이체 급여, 중간정산 대기, 확정 근무자 있는 진행 중 TO
...
export const callableGetWithdrawBlockers = onCall(
  ...
  async (request) => {
    ...
    const hardBlocks: Array<{type: string; count: number}> = [];
    const warnings: Array<{type: string; count: number}> = [];

    if (role === "BUSINESS_ADMIN" || role === "SUPER_ADMIN") {
      // 관리자가 속한 모든 활성 사업장 (managedBusinessIds 아닌 Firestore 직접 쿼리)
      const bizSnap = await db.collection("businesses")
        .where("adminIds", "array-contains", callerUid)
        .get();
      const bizIds = bizSnap.docs
        .filter((d) => !d.data()["deactivatedAt"])
        .map((d) => d.id);
      ...
      // whereIn 최대 30개 배치 처리
      const CHUNK = 30;
      const batches: string[][] = [];
      for (let i = 0; i < bizIds.length; i += CHUNK) {
        batches.push(bizIds.slice(i, i + CHUNK));
      }

      // 병렬 체크
      // [NOTE] Firestore 이중 in 연산자 금지 → 각 wageStatus/status 별 분리 쿼리 또는 in-memory 필터
      const ACTIVE_TO_STATUSES = ["ACTIVE", "FULL", "SCHEDULED"];

      const [wageCount, interimCount, toData, contractCount] = await Promise.all([
        // 1. 미확정(calculated) + 미이체(confirmed) 출근기록 — BLOCK
        // [FIX] in+in 금지 → wageStatus별 분리 쿼리 (businessId in batch × 1 in 씩)
        // [PERF] 직렬 for-loop → Promise.all 병렬 쿼리 전환 (2 × B → 1 라운드)
        // [A-010 FIX] limit(100) → count() 집계 — 100건 초과 시 과소보고 방지
        //   count()는 문서를 읽지 않고 건수만 반환 → 비용 절약 + 정확도 개선
        (async () => {
          const allCounts = await Promise.all(
            (["calculated", "confirmed"] as const).flatMap((ws) =>
              batches.map((batch) =>
                db.collection("attendance")
          ...
          return allCounts.reduce((total, snap) => total + snap.data().count, 0);
        })(),
        ...
        (async () => {
          const allSnaps = await Promise.all(
            batches.map((batch) =>
              db.collection("interim_settlement_requests")
                .where("businessId", "in", batch)
                .where("status", "==", "PENDING")
                .get()
            )
          );
          return allSnaps.reduce((count, snap) => count + snap.size, 0);
        })(),
        ...
        (async () => {
          ...
          for (const snap of allSnaps) {
            for (const doc of snap.docs) {
              ...
              if (!ACTIVE_TO_STATUSES.includes(d["status"] as string)) continue;
              const conf = (d["totalConfirmed"] as number) || 0;
              const pend = (d["totalPending"] as number) || 0;
              confirmedWorkers += conf;
              pendingApplicants += pend;
              if (conf === 0 && pend === 0) emptyActiveTos++;
            }
          }
          return {confirmedWorkers, pendingApplicants, emptyActiveTos};
        })(),

        // 4. 고정계약(isLongTerm) 근로자 활성 계약 — BLOCK
        // 서명 전(pending_*) · 서명 완료(completed) 모두 포함 — voided/expired 제외
        // [NOTE] in+equality 복합 인덱스 불확실 → businessId 단독 조회 후 in-memory 필터
        (async () => {
          const activeStatuses = ["pending_employer", "pending_worker", "completed"];
          const allSnaps = await Promise.all(
            batches.map((batch) =>
              db.collection("employment_contracts")
                .where("businessId", "in", batch)
          ...
          return allSnaps.reduce((count, snap) =>
      ...
      // BLOCK 조건
      if (wageCount > 0) {
        hardBlocks.push({type: "UNPAID_WAGES", count: wageCount});
      }
      if (interimCount > 0) {
        hardBlocks.push({type: "INTERIM_PENDING", count: interimCount});
      }
      if (toData.confirmedWorkers > 0) {
        hardBlocks.push({type: "CONFIRMED_WORKERS", count: toData.confirmedWorkers});
      }
      if (contractCount > 0) {
        hardBlocks.push({type: "ACTIVE_CONTRACTS", count: contractCount});
      }

      // WARNING 조건
      if (toData.pendingApplicants > 0) {
        warnings.push({type: "PENDING_APPLICANTS", count: toData.pendingApplicants});
      }
      if (toData.emptyActiveTos > 0) {
        warnings.push({type: "ACTIVE_TOS", count: toData.emptyActiveTos});
      }

    } else {
      // USER 역할
      const [unsignedCount, pendingWageCount, confirmedWorkCount, subAdminBizCount] = await Promise.all([
        // 1. 미서명 계약서 — BLOCK
        // VOID/EXPIRED 제외: EXPIRED는 서명 불가 상태이므로 영구 해소 불가능한 BLOCK 방지
        (async () => {
          const snap = await db.collection("employment_contracts")
            .where("workerId", "==", callerUid)
            .where("workerSignedAt", "==", null)
            .get();
          const blockedStatuses = ["VOID", "EXPIRED"];
        ...
        // [FIX] 'applicantId' → 'uid' (Firestore 실제 필드명)
        // [FIX] 'type' == 'long_term' → 'isLongTermApplication' == true (인덱스 있음)
        (async () => {
          const snap = await db.collection("applications")
            .where("uid", "==", callerUid)
            .where("status", "==", "CONFIRMED")
            .where("isLongTermApplication", "==", true)
            .get();
          return snap.size;
        })(),

        // 4. 서브관리자 등록 사업장 수 — BLOCK (회원탈퇴 전 서브관리자 직책 해제 필요)
        (async () => {
          const snap = await db.collection("users").doc(callerUid).get();
          const ids = (snap.data()?.subAdminBusinessIds ?? []) as string[];
          return ids.length;
        })(),
      ]);

      if (unsignedCount > 0) {
        hardBlocks.push({type: "UNSIGNED_CONTRACT", count: unsignedCount});
      }
      if (pendingWageCount > 0) {
        warnings.push({type: "PENDING_WAGES", count: pendingWageCount});
      }
      if (confirmedWorkCount > 0) {
        warnings.push({type: "CONFIRMED_WORK", count: confirmedWorkCount});
      }
      if (subAdminBizCount > 0) {
        hardBlocks.push({type: "SUBADMIN_MEMBERSHIP", count: subAdminBizCount});
      }
    }
...
export const callableGetUserByPhone = onCall(
  ...
  async (request) => {
    ...
    const isSubAdmin = callerRole === "USER" && ((callerData.subAdminBusinessIds ?? []) as string[]).length > 0;
...
export const callableRequestInterimSettlement = onCall(
  ...
  async (request) => {
    ...
    // [PAY-CF01] 0원 요청 허용 시 중복 PENDING으로 정상 요청이 차단되는 버그 방지
    if ((d.requestedAmount ?? 0) <= 0 || (d.netAmount ?? 0) <= 0)
      throw new HttpsError("invalid-argument", "금액은 1 이상이어야 합니다.");
    ...
    // [BUG-01] 클라이언트 제공 requestedAmount/netAmount 무시 — 서버 측에서 finalWage/netWage 합산
    const invalidIds: string[] = [];
    let serverRequestedAmount = 0;
    let serverNetAmount = 0;
    for (const snap of attSnaps) {
      ...
      if (ad.userId !== callerUid || ad.businessId !== d.businessId || ad.wageStatus !== "confirmed") {
        ...
        const wage = (ad.finalWage as number | undefined) ?? 0;
        serverRequestedAmount += wage;
        serverNetAmount += (ad.netWage as number | undefined) ?? wage;
      }
    }
    if (invalidIds.length > 0)
      throw new HttpsError("invalid-argument", `유효하지 않은 출근기록: ${invalidIds.join(", ")}`);
    if (serverRequestedAmount <= 0)
      throw new HttpsError("failed-precondition", "확정된 임금이 없어 중간정산 요청이 불가합니다.");

    // 3+4. [PAY-DUP01] TOCTOU 방지 — 중복 체크와 문서 생성을 트랜잭션으로 원자화
    const now = admin.firestore.Timestamp.now();
    const docRef = db.collection("interim_settlement_requests").doc();
    await db.runTransaction(async (tx) => {
      const dupQuery = db.collection("interim_settlement_requests")
        .where("workerId", "==", callerUid)
      ...
      tx.set(docRef, {
        ...
        requestedAmount: Math.round(serverRequestedAmount),
        netAmount: Math.round(serverNetAmount),
    ...
    // [ISR-SELF-NOTIFY] 요청자(근로자)가 adminIdSet에 포함된 경우 자기발송 방지
    adminIdSet.delete(callerUid);
    // [FCM-BUG-E] canManageWage 서브어드민에게도 팬아웃 (callableApplyToTO 패턴 참조)
    try {
      const subAdmins = await getSubAdminsWithPermission(d.businessId, "canManageWage");
      subAdmins.forEach((id) => { if (!adminIdSet.has(id)) adminIdSet.add(id); });
    } catch (e) {
      console.warn("[callableRequestInterimSettlement] 서브어드민 조회 실패 (무시):", e);
    }
...
export const callableApproveInterimSettlement = onCall(
  ...
  async (request) => {
    ...
    // [BUG-02] "YYYY-MM-DD" → new Date() 파싱이 환경에 따라 로컬/UTC가 달라질 수 있음
    // → T00:00:00Z 명시로 항상 UTC 자정 파싱 보장 (FCM 날짜 표시 일치)
    const transferDate = new Date(d.scheduledTransferDate + "T00:00:00Z");
    if (isNaN(transferDate.getTime()))
      throw new HttpsError("invalid-argument", "scheduledTransferDate 형식 오류 (YYYY-MM-DD)");

    const {callerData: approveCallerData, bizData: approveBizData} = await assertBizAdmin(callerUid, d.businessId);
    // [ISR-APPROVE-PERM] 서브어드민 canManageWage 세부 권한 검증
    ...
    if (
      ...
      (approveBizData?.ownerId as string | undefined) !== callerUid
    ) {
...
export const callableProcessInterimSettlement = onCall(
  ...
  async (request) => {
    ...
    const {callerData: processCallerData, bizData: processBizData} = await assertBizAdmin(callerUid, d.businessId);
    // [ISR-APPROVE-PERM] 서브어드민 canManageWage 세부 권한 검증
    const processAdminIds = (processBizData?.adminIds as string[] | undefined) ?? [];
    if (
      (processCallerData?.role as string | undefined) !== "SUPER_ADMIN" &&
      !processAdminIds.includes(callerUid) &&
      (processBizData?.ownerId as string | undefined) !== callerUid
    ) {
...
// ─── callableGetPublishedTOs ───────────────────────────────────────────────
// [CF-MIGRATED D6-CP2] tos list isPublished 필터 서버 강제
// Firestore rules list: USER limit<=50 허용 → 미발행 TO 노출 가능 → CF로 이전
// Admin SDK 사용 → rules 우회 + isPublished/status 복합 쿼리 가능
export const callableGetPublishedTOs = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const {type, city, district} = (request.data ?? {}) as {
      type?: string;
      city?: string;
      district?: string;
    };
    ...
    if (city) q = q.where("businessCity", "==", city);
    if (city && district) q = q.where("businessDistrict", "==", district);

    q = q.limit(50);
    const snap = await q.get();
    const tos = snap.docs.map((d) => ({id: d.id, ...serializeFirestoreData(d.data())}));
    return {tos};
  }
);

// ─── callableGetPublishedTOsPaged ─────────────────────────────────────────
// [CF-MIGRATED D6-CP2] tos list 페이지네이션 — isPublished 서버 강제
// DocumentSnapshot 커서 → lastToId(String) 커서로 전환
export const callableGetPublishedTOsPaged = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const {type, city, district, pageSize: rawSize, lastToId} = (request.data ?? {}) as {
      type?: string;
      city?: string;
      district?: string;
      pageSize?: number;
    ...
    const pageSize = Math.min(rawSize ?? 30, 100);
    ...
    if (lastToId) {
      const cursorSnap = await db.collection("tos").doc(lastToId).get();
      if (!cursorSnap.exists) {
        return {tos: [], hasMore: false, lastToId: null};
      }
      q = q.startAfter(cursorSnap);
    }
    ...
    const hasMore = snap.docs.length > pageSize;
    const docs = hasMore ? snap.docs.slice(0, pageSize) : snap.docs;
    const tos = docs.map((d) => ({id: d.id, ...serializeFirestoreData(d.data())}));
    const nextLastToId = docs.length > 0 ? docs[docs.length - 1].id : null;

    return {tos, hasMore, lastToId: nextLastToId};
  }
);

// ─── callableRemoveMember ─────────────────────────────────────────────────────
// [CF-MIGRATED] Trust Boundary Charter — subAdminBusinessIds는 _protectedUserFields
// 클라이언트 직접 users/{uid} 쓰기에서 CF로 이전 (member_service.dart removeMember)
// BUSINESS_ADMIN 전용 (CLAUDE.md: 멤버 관리는 SubAdmin 완전 제외)
export const callableRemoveMember = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    const {businessId, uid} = (request.data ?? {}) as {businessId?: string; uid?: string};
    if (!businessId || typeof businessId !== "string") {
      throw new HttpsError("invalid-argument", "businessId가 필요합니다.");
    }
    ...
    // ── 트랜잭션: 멤버 삭제 + subAdminBusinessIds 서버 제거 ─────────────────────
    const userRef = db.collection("users").doc(uid);
    await db.runTransaction(async (tx) => {
      const userSnap = await tx.get(userRef);
      tx.delete(memberRef);
      ...
      const legacySubAdminOf = userData.subAdminOf as string | undefined;
      const isSubAdminOfThisBiz =
        subAdminIds.includes(businessId) || legacySubAdminOf === businessId;
...
// 서브관리자가 특정 사업장에서 스스로 직책 해제 (회원탈퇴 차단 해소 경로)
// Trust Boundary: 타인(businesses/members) + 자신의 _protected 필드 수정 → CF 필수
// Input : { businessId }
// Output: { success }
export const callableLeaveAsSubAdmin = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    ...
    await db.runTransaction(async (tx) => {
      ...
      const subAdminIds = (userSnap.data()?.subAdminBusinessIds ?? []) as string[];
      if (!subAdminIds.includes(businessId)) {
        // 이미 해제된 상태이면 멱등 처리 (에러 없이 성공 반환)
        return;
      }

      // 1. members 서브컬렉션 삭제 (존재하지 않아도 무해)
      const memberSnap = await tx.get(memberRef);
      if (memberSnap.exists) {
        tx.delete(memberRef);
      }

      // 2. users.subAdminBusinessIds 에서 해당 businessId 제거
      tx.update(userRef, {
        subAdminBusinessIds: admin.firestore.FieldValue.arrayRemove(businessId),
      });
    });
...
// ── callableCancelTOInvitation ────────────────────────────────────────────────
// 관리자가 발송한 TO 초대를 철회한다. INVITED → CANCELED
// 초대를 보낸 관리자(invitedBy) 또는 사업장 관리자만 취소 가능.
// Input : { applicationId }
// Output: { success }
export const callableCancelTOInvitation = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    ...
    // ── 2. 상태 검증 ─────────────────────────────────────────────────────────
    const currentStatus = appData.status as string | undefined;
    if (currentStatus === "CANCELED") return {success: true};
    if (currentStatus === "CONFIRMED") {
      throw new HttpsError("failed-precondition", "이미 수락된 초대는 취소할 수 없습니다.");
    }
    if (currentStatus !== "INVITED" && currentStatus !== "EXPIRED") {
      throw new HttpsError("failed-precondition", `취소할 수 없는 상태입니다: ${currentStatus}`);
    }
    ...
    // ── 4. 근로자에게 취소 알림 ─────────────────────────────────────────────
    try {
      const targetUid = appData.uid          as string | undefined;
      const toTitle   = (appData.toTitle     as string | undefined) ?? "";
      const bizName   = (appData.businessName as string | undefined) ?? "";
      if (targetUid) {
        const body = bizName
          ? `${bizName}에서 '${toTitle}' 근무 초대를 취소했습니다.`
          : `'${toTitle}' 근무 초대가 취소되었습니다.`;
        db.collection("users").doc(targetUid).collection("notifications").add({
          userId:    targetUid,
          type:      "toInviteCanceled",
          title:     "초대가 취소되었습니다",
          body,
          data:      {businessId, action: "myApplications"},
          isRead:    false,
          createdAt: admin.firestore.FieldValue.serverTimestamp(),
          readAt:    null,
        }).catch((err) => console.error("[callableCancelTOInvitation] 근로자 알림 실패:", err));
      }
    } catch (_) { /* 알림 실패는 취소 결과에 영향 없음 */ }
...
// - 권한: assertBizAdmin 검증 (서브어드민 canManageWage 포함)
// - 원자성: Firestore 트랜잭션으로 attendance 이체 + 요청 문서 생성 동시 처리
// - 멱등: 이미 transferred인 attendance는 건너뜀
export const callableAdminDirectInterimSettlement = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    ...
    const {
      businessId, workerId, workerName, businessName,
      applicationId, attendanceIds, netAmount,
    } = request.data as {
      businessId: string;
      workerId: string;
      workerName: string;
    ...
    // 권한 검증
    const {callerData: adminCallerData} = await assertBizAdmin(callerUid, businessId);
    const isSubAdmin = ((adminCallerData?.subAdminBusinessIds ?? []) as string[]).includes(businessId);
    if (isSubAdmin) {
      const memberSnap = await db.collection("businesses").doc(businessId)
        .collection("members").doc(callerUid).get();
      const perms = (memberSnap.data()?.permissions ?? {}) as Record<string, boolean>;
      if (perms.canManageWage !== true) {
        throw new HttpsError("permission-denied", "급여 관리 권한이 없습니다.");
    ...
    await db.runTransaction(async (tx) => {
      ...
      // interim_settlement_requests 문서 생성 (PROCESSED 상태로 직접 기록)
      tx.set(settlementRef, {
        ...
        applicationId,
        attendanceIds,
        requestedAmount: netAmount ?? actualNetAmount,
    ...
    // FCM 알림 (실패해도 이체에 영향 없음)
    try {
      if (actualNetAmount > 0) {
        await db.collection("users").doc(workerId).collection("notifications").add({
          userId:   workerId,
          ...
          data: {
            businessId,
            screen: "interimSettlement",
            settlementRequestId: settlementRef.id,
          },
          createdAt: now,
          readAt:    null,
        });
      ...
      console.error("[callableAdminDirectInterimSettlement] 알림 발송 실패:", e);
    }

    return {success: true, settlementId: settlementRef.id, netAmount: actualNetAmount};
  }
);

// ─── callableRecalcToTotalRequired ────────────────────────────────────────────
// 슬롯 totalRequired 필드 합산으로 TO.totalRequired를 서버에서 재계산
// [Trust Boundary] 클라이언트 FieldValue.increment(delta) 경합 조건 대체
// - 슬롯 수천 건에 대응: 499건 페이지네이션
// - 원자성: 최종 합산값만 TO 문서에 1회 업데이트
export const callableRecalcToTotalRequired = onCall(
  {region: "asia-northeast3", enforceAppCheck: true},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    ...
    let totalRequired = 0;
    let lastSlot: FirebaseFirestore.DocumentSnapshot | undefined;
    while (true) {
      let q: FirebaseFirestore.Query = db
        .collection("tos").doc(toId).collection("slots")
        .limit(499);
      if (lastSlot) q = q.startAfter(lastSlot);
      const pageSnap = await q.get();
      if (pageSnap.empty) break;
      for (const doc of pageSnap.docs) {
        totalRequired += ((doc.data().totalRequired as number | undefined) ?? 0);
      }
      if (pageSnap.size < 499) break;
      lastSlot = pageSnap.docs[pageSnap.docs.length - 1];
    }

    // TO.totalRequired 업데이트
    await db.collection("tos").doc(toId).update({
      totalRequired,
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    });

    return {success: true, totalRequired};
  }
);

// ═══════════════════════════════════════════════════════════
// 🔒 [SEC-FIX 2026-08-10] callableGetWorkerBasicProfile
//    관리자/서브관리자가 계약서 작성 등을 위해 근무자 기본 정보 조회
//    - assertBizAdmin(callerUid, businessId) 검증 (BUSINESS_ADMIN + SubAdmin 포함)
//    - 해당 근무자가 해당 사업장의 지원자인지 applications 컬렉션으로 재검증
//    - 민감 필드(residentNumber, ci, foreignIdNumber, accountNumber) 제외하고 반환
// ═══════════════════════════════════════════════════════════
export const callableGetWorkerBasicProfile = onCall(
  {region: "asia-northeast3"},
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const callerUid = request.auth.uid;
    const {workerUid, businessId} = request.data as {workerUid?: string; businessId?: string};

    if (!workerUid || !businessId) {
      throw new HttpsError("invalid-argument", "workerUid, businessId가 필요합니다.");
    }

    // 1. 권한 검증 — BUSINESS_ADMIN(ownerId/adminIds) + SubAdmin(subAdminBusinessIds) 모두 허용
    await assertBizAdmin(callerUid, businessId);

    // 2. 해당 근무자가 해당 사업장 지원자인지 확인 (applications 컬렉션)
    ...
    if (appSnap.empty) {
      throw new HttpsError(
        "permission-denied",
        "해당 사업장의 지원자 정보에 접근할 권한이 없습니다."
      );
    }

    // 3. 민감 필드 제외한 기본 정보만 반환
    const workerDoc = await db.collection("users").doc(workerUid).get();
    if (!workerDoc.exists) {
      throw new HttpsError("not-found", "근무자 정보를 찾을 수 없습니다.");
    }

    const wd = workerDoc.data()!;
    const birthDateTs = wd["birthDate"] as admin.firestore.Timestamp | undefined;

    return {
      name:          (wd["name"]          as string | undefined) ?? null,
      phone:         (wd["phone"]         as string | undefined) ?? null,
